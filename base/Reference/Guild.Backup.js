...
const GuildCategory = require("../../database/Schemas/Guards/Backup/Guild.Category.Channels");
const GuildText = require("../../database/Schemas/Guards/Backup/Guild.Text.Channels");
const GuildVoice = require("../../database/Schemas/Guards/Backup/Guild.Voice.Channels");
        ...
        if (guild) {
            ...
            for (let index = 0; index < channels.length; index++) {
                ...
                if ((channel.type === ChannelType.GuildText) || (channel.type === 'ChannelType.GuildNews')) {
                    await GuildText.findOne({ channelID: channel.id }, async (err, kanalYedek) => {
                        if (!kanalYedek) {
                            const newData = new GuildText({
                ...
                if (channel.type === ChannelType.GuildText) {
                    await GuildVoice.findOne({ channelID: channel.id }, async (err, kanalYedek) => {
                        if (!kanalYedek) {
                            const newData = new GuildVoice({
                                channelID: channel.id,
                                name: channel.name,
                ...
                if (channel.type === ChannelType.GuildText) {
                    await GuildCategory.findOne({ channelID: channel.id }, async (err, kanalYedek) => {
                        if (!kanalYedek) {
                            const newData = new GuildCategory({
                                channelID: channel.id,
                                name: channel.name,
                                position: channel.position,
                                overwrites: ChannelPermissions,

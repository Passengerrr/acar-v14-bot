const { Client,  GatewayIntentBits, Partials, Collection, Constants, Intents, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, Options} = require("discord.js");
      ...
      constructor (options) {
       ...
       async startDistributors() {
        ...
        async checkMember(id, type, process = "İşlem Bulunamadı.") {
            ...
            if(cartelcim.id === this.user.id || cartelcim.id === cartelcim.guild.ownerId || Whitelist.unManageable.some(g => cartelcim.id === g || cartelcim.roles.cache.has(g)) || Whitelist.BOTS.some(g => cartelcim.id === g || cartelcim.roles.cache.has(g))|| guildSettings.staff.includes(cartelcim.id)) return true; 
        ...
        async checkProcessLimit(cartelcim, limit, zaman, process) {
            ...
            if (limitController.length >= limit) { 
                let loged = cartelcim.guild.kanalBul("guard-log");
...
${limitController.sort((a, b) => b.date - a.date).map((x, index) => `${index+1}. | ${x.proc} | ${tarihsel(x.date)}`).join("\n")}
                  ...
                  return new Promise(async (resolve) => { 
                      for (let index = 0; index < members.length; index++) {
                          ...
                          await cartelcim.roles.add(role.id).catch(() => {
                              client.logger.log(`${cartelcim.user.tag} - Üyesine rol verilemedi.`, "log");
                          })
                      }
                      resolve();
                  })
        ...
        async allPermissionClose() {
            ...
            if(roller) {
                ...
                if(owner) owner.send({embeds: [new EmbedBuilder().setColor("Random").setDescription(`**Merhaba!** ${sunucu.name}
Aşağı da listelenmekte olan rol(lerin) önemli izinleri başarıyla kapatıldı. ${sunucu.emojiGöster(emojiler.onay_cartel) ? sunucu.emojiGöster(emojiler.onay_cartel) : ":white_check_mark:"}
...
Yukarda bulunan rol(lerin) izinlerini tekrardan aktif etmek için, ${sunucu.name} sunucusunda bulunan \`#guard-log\` kanalında ki düğmeden aktif edebilirsin.`)]}).catch(err => {})
                ...
                if(kanal) kanal.send({content:`@everyone`, embeds: [new EmbedBuilder().setColor("Random").setDescription(`**Merhaba!** ${sunucu.name}
Aşağı da listelenmekte olan rol(lerin) önemli izinleri başarıyla kapatıldı. ${sunucu.emojiGöster(emojiler.onay_cartel) ? sunucu.emojiGöster(emojiler.onay_cartel) : ":white_check_mark:"}
...
Aşağıda bulunan düğme ile tekrardan aktif edebilirsin. Bunun için sunucu sahibi veya bot sahibi olmalısın.`)], components: [Rows]}).then(async (msg) => {
                    ...
                    collector.on('collect', async (i) => {
                        ...
                        Rows.components[0].setStyle(ButtonStyle.Success).setLabel("Rol İzinleri Aktif Edildi").setDisabled(true)
                        msg.edit({components: [Rows]})
                        i.reply({embeds: [new EmbedBuilder().setColor("WHITE").setDescription(`
Başarıyla ${sunucu.name} sunucusunun **${checkRoles ? checkRoles.length >= 1 ? checkRoles.length : 0 : 0}** rolünün izinleri tekrardan aktif edildi. ${sunucu.emojiGöster(emojiler.onay_cartel) ? sunucu.emojiGöster(emojiler.onay_cartel) : ":white_check_mark:"}

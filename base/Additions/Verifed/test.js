const {
   ...
   * Embed seçilir ise sayfalara EmbedBuilder ile embed oluşturulmalıdır. 
  ...
  const pageEmbed = async (
    ...
    const finishRow = new ActionRowBuilder().addComponents(buttons[1]);
    ...
    if(type == "EMBED") {
       curPage = await interaction.editReply({
            embeds: [pages[page].setFooter({text:detail ? { text: `Sayfa ${page + 1} / ${pages.length}` } : null})],
            components: [row],
            fetchReply: true,
        });

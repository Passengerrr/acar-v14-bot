const { GuildMember, Guild, TextChannel, Message, EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, ChannelType } = require("discord.js");
...
// Schema's
    ...
    const Leaders = require('../../database/Schemas/Plugins/Guild.Responsibility');
...
Sana İyi Sohbetler Dilerim! :tada: :tada: :tada:`}).catch(err => {})
    ...
    if(kayıtLog) kayıtLog.send({embeds: [new EmbedBuilder().setDescription(`${this} kullanıcısı ${registrant}isimli yetkili tarafından <t:${String(Date.now()).slice(0, 10)}:R> **${gender}** olarak \`${this.guild.name}\` sunucusuna kayıt işlemi gerçekleştirildi.`)]})
...
${this} (**${this.user.tag}**)
ID: ${this.id}`, embeds: [new EmbedBuilder()
                ...
                .setFooter({text:"bu bilgilendirmeye mesajına uyarak, lütfen eski yetkilini sunucuya tekrardan davet et."})]}).catch(err => { })
...
Düşüm Sonrası Yetkili Ceza Notu **0** Altına İndiğinde Otomatik Olarak Yetkisi Çekilecektir. Yetkili Ceza Notu: \`${yetkiliCezaPuanı}\` (Anlık Ceza Puanı: \`${cezaPuanı}\`)`})
            ...
            if((roller.Yetkiler && !roller.Yetkiler.some(x => this.roles.cache.has(x))) && cezaPuanı >= 50) {
                ...
                if(logKanalı) logKanalı.send({embeds: [new EmbedBuilder().setDescription(`${this} Üyesi **50** Ceza Puanını ${güm ? `Geçtiği İçin Otomatik Olarak Şüpheli Hesap Konumuna Gönderildi.` : `Geçti Fakat Son Atılan Cezası Şüpheli Konumundan Daha Üstün Olduğu İçin Gönderilemedi.`} Anlık Ceza Puanı: \`${cezaPuanı}\``)]})

            } else if((roller.Yetkiler && !roller.Yetkiler.some(x => this.roles.cache.has(x)))){
                if(logKanalı) logKanalı.send({content: `${this} Kişisinin Ceza Puanı Güncellendi, Yeni Puanı: ${cezaPuanı}`})
            }
        }
        return cezaPuanı;
}
    ...
    if(userData && userData.Staff && userData.StaffGiveAdmin) {
        let Yetkili = await Users.findOne({_id: userData.StaffGiveAdmin }) || {}
          if(Yetkili && Yetkili.Staffs) {
              ...
              if(BabanıSikeyim) BabanıSikeyim.send({embeds: [new EmbedBuilder().setDescription(`${BabanıSikeyim} Merhaba, ${this} (\`${this.id}\`) isimli çektiğin yetkili yetkiyi sunucudan çıkarak <t:${String(Date.now()).slice(0, 10)}:R> bıraktı.`).setFooter({text:"bu bilgilendirmeye mesajına uyarak, lütfen eski yetkilini sunucuya tekrardan davet et."})]}).catch(err => { })
              const findUser = Yetkili.Staffs.find(cartel => cartel.id == this.id);
              await Users.updateOne({ _id: userData.StaffGiveAdmin }, { $pull: { "Staffs": findUser } }, { upsert: true })
              client.Upstaffs.removePoint(userData.StaffGiveAdmin, -_statSystem.points.staff, "Yetkili")
          } 
...
GuildMember.prototype.Leaders = async function(text, point, option = {
    ...
    if(roles && roles.length > 0 && roles.some(b => this.roles.cache.has(b.role))) {
        ...
        for (let index = 0; index < roles.length; index++) {
            ...
            .setAuthor({name: `${this.displayName} ${header} Puan'ı Kazandı!`, iconURL: this.user.displayAvatarURL({dynamic: true})})
            .setDescription(`
...
Yapılan İşlem Türü İse "**${detay.type}**" Olarak Tespit Edildi.`)
            if(option.user)  {
                let target = this.guild.members.cache.get(option.user)
                embed.setColor("Random").addField(`İşlem Yapılan Kullanıcı`, `${ target ? `${target} (\`${target.id}\`)` : `<@${option.user}> (\`Sunucudan Ayrılmış\`)`}`)
            }
            if(option.channel)  {
                let target = this.guild.channels.cache.get(option.channel) || this.guild.channels.cache.get(option.channel.id)
                embed.setColor("Random").addField(`İşlem Yapılan Kanal`, `${target ? `${target}` : `@deleted-channel`}`)
            }
            if(option.role)  {
                let target = this.guild.roles.cache.get(option.role)
                embed.setColor("Random").addField(`İşlem Yapılan Rol`, `${target ? `${target}` : `@deleted-roles`}`)
            }
...
GuildMember.prototype.addPunitives = async function(type, staff, reason = "Sebep belirtilmedi.", message, duration, muteFlood, rulesFlood, nofi = 1) {
    ...
    cezano = cezano == 0 ? 1 : cezano + 1;
        ...
        if(nofi == 0 && !muteFlood && !rulesFlood) await message.editReply({embeds: [new EmbedBuilder().setDescription(`${this.toString()} isimli üyeye **${reason}** sebebiyle ${duration ? `\`${moment.duration(ms(duration)).format('Y [Yıl,] M [Ay,] d [Gün,] h [Saat,] m [Dakika]')}\` boyunca` : ``} ${type == "Kalkmaz Yasaklama" ? 
        "tamamiyle kalkmaz yasaklamaya tabi tutuldun."
    : type == "Yasaklama" ? 
        "yasakladın."
    : type == "Cezalandırılma" ? 
    ...
    : type == "Uyarılma" ?
    "uyardın."
    : `"${type}" türünde ceza-i işlem uygulandı.`} ${message.guild.emojiGöster(emojiler.onay_cartel)}`).setFooter({text:`${this.user.tag} • Ceza Numarası: #${cezano} • ${type}`, iconURL: this.user.avatarURL({dynamic: true})})], components: []})
        switch(type) {

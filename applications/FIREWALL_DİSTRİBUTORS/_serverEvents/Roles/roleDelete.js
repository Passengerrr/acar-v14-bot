...
module.exports = async role => {
  ...
  if(Data && !Data.roleGuard) return;
    let embed = new EmbedBuilder().setTitle("Sunucuda Rol Silindi!")
    ...
    if(arr.includes(role.id)) {
        embed.setColor("Random").setTitle("Sunucuda Özel Bir Rol Silindi!")
        ...
        embed.setColor("Random").setDescription(`${entry.executor} (\`${entry.executor.id}\`) tarafından **@${role.name}** (\`${role.id}\`) isimli özel rol silindi, silen kişi sunucudan uzaklaştırılarak silinen rol tekrardan kurulmaya başlandı.
    
**Rol İsmi**: \` @${newRole.name} \` [${newRole}]
**Dağıtılacak Kişi Bilgisi**: \` ${roleData.members.length || 0} kişi \`
**Tahmini Dağıtım Süresi**: \` ${roleData ? roleData.members.length > 0 ? (roleData.members.length>1000 ? parseInt((roleData.members.length*(250/1000)) / 60)+" dakika" : parseInt(roleData.members.length*(250/1000))+" saniye") : "Belirlenemedi." : "Belirlenemedi." } \``).setFooter({text:"yukarda belirtilen rol bilgisi dahilinde tekrardan kurulum sağlanmıştır ve dağıtımı başlamıştır."});
    ...
    embed.setColor("Random").setDescription(`${entry.executor} (\`${entry.executor.id}\`) tarafından **@${role.name}** (\`${role.id}\`) isimli rol silindi, silen kişi sunucudan uzaklaştırılarak silinen rol tekrardan kurulmaya başlandı.

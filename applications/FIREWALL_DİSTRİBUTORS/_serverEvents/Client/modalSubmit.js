...
const { Client, Message, Util, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, Collection, Permissions, EmbedBuilder } = Discord = require("discord.js");
...
module.exports = async (modal) => {
    ...
    if(modal.customId == "sunucuDüzenleme") {
        ...
        if(gerçekleşenİşlemler.length > 0) {
            ...
            if(logKanal) logKanal.send({embeds: [new EmbedBuilder().setColor("ORANGE")
    .setFooter({text:cartelim.user.tag + " tarafından güncellendi.", iconURL:cartelim.user.avatarURL({dynamic: true})})
...
\`\`\``)]})
            await modal.deferReply({ ephemeral: true })
            return await modal.followUp({embeds: [new EmbedBuilder().setColor("Random").setDescription(`Başarıyla ${guild.name} sunucusu üzerinde işlem(ler) <t:${String(Date.now()).slice(0, 10)}:F> tarihinde gerçekleştirildi/güncelleştirildi.

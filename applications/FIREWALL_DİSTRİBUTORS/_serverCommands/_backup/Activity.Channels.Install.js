...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    CategoryChannels.findOne({ channelID: args[0] }, async (err, data) => {
      if (!data) return message.channel.send(`${cevaplar.prefix} Belirtilen kategori kanalı geçmişte bulunamadığından işlem iptal edildi `), message.react(message.guild.emojiGöster(emojiler.no_cartel) ? message.guild.emojiGöster(emojiler.no_cartel).id : undefined)
      const newChannel = await message.guild.channels.create(data.name, {
        type: ChannelType.GuildText,
        position: data.position,
      });
      await message.channel.send({ embeds: [embed.setColor("Random").setColor("Random").setDescription(`**${newChannel.name}** isimli kategorinin, \`${tarihsel(Date.now())}\` tarihli kategori yedeği kurulmaya başladı.`)]})
...
async function voiceKur(idcik, message, embed, parentID) {
    VoiceChannels.findOne({ channelID: idcik }, async (err, data) => {
        if (!data) return message.channel.send(`${cevaplar.prefix} Belirtilen ses kanal geçmişte bulunamadığından işlem iptal edildi `), message.react(message.guild.emojiGöster(emojiler.no_cartel) ? message.guild.emojiGöster(emojiler.no_cartel).id : undefined)
        const newChannel = await message.guild.channels.create(data.name, {
          type: ChannelType.GuildText,
...
async function textKur(idcik, message, embed, parentID) {
    TextChannels.findOne({ channelID: idcik }, async (err, data) => {
        if (!data) return message.channel.send(`${cevaplar.prefix} Belirtilen metin kanal geçmişte bulunamadığından işlem iptal edildi `), message.react(message.guild.emojiGöster(emojiler.no_cartel) ? message.guild.emojiGöster(emojiler.no_cartel).id : undefined)
        const newChannel = await message.guild.channels.create(data.name, {
          type: ChannelType.GuildText,
          nsfw: data.nsfw,
          parentID: parentID,

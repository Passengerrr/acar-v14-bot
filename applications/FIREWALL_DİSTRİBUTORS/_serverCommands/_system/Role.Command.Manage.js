const { Client, Message, ButtonBuilder, ButtonStyle, ActionRowBuilder, StringSelectMenuBuilder, EmbedBuilder  } = require("discord.js");
  ...
  onLoad: function (client) {
    client.on('modalSubmit', async (modal) => {
      ...
      if(modal.customId == "tp-kaldır") {
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({content: `Başarıyla ${cmdName} komutu <t:${String(Date.now()).slice(0, 10)}:R> kaldırıldı. ${guild.emojiGöster(emojiler.onay_cartel)}` , ephemeral: true })
      ...
      if(modal.customId == "tp-detay") {
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({embeds: [new EmbedBuilder().setThumbnail(guild.iconURL({dynamic: true})).setDescription(`Aşağı da **${cmdName}** isimli rol (ver/al) veya alt komutun detaylı bilgileri belirtilmiştir.\n
...
${cmd.Permission ? cmd.Permission.map(x => guild.roles.cache.get(x)).join(", ") : "@rol bulanamadı"} rol veya rolleri kullanabilir.`)] , ephemeral: true })
      ...
      if(modal.customId == "tp-ekle") {
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({content: `Başarıyla ${cmdName} komutu <t:${String(Date.now()).slice(0, 10)}:R> eklendi. ${guild.emojiGöster(emojiler.onay_cartel)}` , ephemeral: true })
      ...
      collector.on('end', (collected, reason) => {
          if(reason == "time") {
            ...
            load.edit({components: [Row], embeds: [
              new EmbedBuilder().setDescription(`Zaman aşımına uğradığı için işleminiz sonlandırıldı. ${cevaplar.prefix}`)
            ]});
            setTimeout(() => {

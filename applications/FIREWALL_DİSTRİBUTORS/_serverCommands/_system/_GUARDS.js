const { Client, Message, Util, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder} = require("discord.js");
...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let anaMenu = new EmbedBuilder().setColor("Random")
        ...
        message.channel.send({embeds: [anaMenu], components: [oneRows]}).then(async (msg) => {
            ...
            collector.on('collect', async (i) => {
              ...
              if(i.customId == "guard_manager") {
                ...
                let münürRow = new ActionRowBuilder().addComponents([
                    new ButtonBuilder()
                    ...
                    collector.on('collect', async (i) => {
                        ...
                        if(i.customId == "guardState") {
                            ...
                            if(guardSettings.guildProtection) {
                                ...
                                münürRow.components[0].setStyle(ButtonStyle.Danger).setLabel(`Güvenliği Aç`)
                               ...
                               if(ayarlar.SMS) münürRow.components[4].setDisabled(false)
                                münürRow.components[0].setStyle(ButtonStyle.Secondary).setLabel(`Güvenliği Kapat`)
                        ...
                        if(i.customId == "limitState") {
                            ...
                            if(guardSettings.limit) {
                                münürRow.components[2].setDisabled(true)
                                münürRow.components[1].setStyle(ButtonStyle.Danger).setLabel(`Limiti Aç`)
                                ...
                                münürRow.components[1].setStyle(ButtonStyle.Secondary).setLabel(`Limiti Kapat`)
                        ...
                        if(i.customId == "limitSettings") {
                            ...
                            message.channel.send({embeds: [new EmbedBuilder().setColor("Random")
...
Olarak ayarlanmış. Ayarı güncellemek için dakika cinsinde işlem süresi belirtin.`)
                            ]}).then(async (ymsg) => {
...
Olarak ayarlanmış fakat işlem sayısı ayarlanmamış. Ayarlamak için lütfen kaç işlem olacağını belirtin.`)]}).then(async (yarramm) => {
    var filter = (m) => m.author.id == message.member.id
    ...
    collector.on('collect', async (m) => {
        ...
        let guardSettings = await GUARD_SETTINGS.findOne({guildID: sistem.SUNUCU.GUILD})
        yarramm.edit({embeds: [new EmbedBuilder().setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla sunucunun limit ayarları güncellendi.
    ...
    collector.on('collect', async (i) => {
        ...
        i.deferUpdate().catch(err => {})
                        ...
                        if(i.customId == "smsState") {
                            ...
                            if(guildSettings) ayarlar = guildSettings.Ayarlar
                            if(ayarlar.SMS) {
                                münürRow.components[3].setStyle(ButtonStyle.Danger).setLabel(`SMS/MMS Aç`)
                                münürRow.components[4].setDisabled(true).setStyle(ButtonStyle.Success).setLabel(`SMS/MMS Gerekli`)
                                ...
                                münürRow.components[4].setDisabled(false).setLabel(`WhatsApp/Telegram Aç`).setStyle(ButtonStyle.Danger)
                                münürRow.components[3].setStyle(ButtonStyle.Secondary).setLabel(`SMS/MMS Kapat`)
                        ...
                        if(i.customId == "wpState") {
                            ...
                            if(ayarlar.WHATSAPP | ayarlar.TELEGRAM) {
                                münürRow.components[4].setStyle(ButtonStyle.Danger).setLabel(`WhatsApp/Telegram Aç`)
                                await GUILD_SETTINGS.updateOne({guildID: sistem.SUNUCU.GUILD}, { $set: {"Ayarlar.WHATSAPP": false, "Ayarlar.TELEGRAM": false }}, {upsert: true})
                                
                            } else if(!ayarlar.WHATSAPP || !ayarlar.TELEGRAM) {
                                münürRow.components[4].setStyle(ButtonStyle.Secondary).setLabel(`WhatsApp/Telegram Kapat`)
...
.addField("Chat Beyaz Liste", `${ayarlar.chatİzinliler.length >= 1 ? ayarlar.chatİzinliler.map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x) || message.guild.members.cache.get(x) || message.guild.channels.cache.get(x)  || x}`).join("\n") : 'Listede sonuç bulunamadı!'}`, false)
        ...
        message.channel.send({embeds: [new EmbedBuilder().setDescription(`**Merhaba!** ${message.member.user.tag}

const {ButtonBuilder, ActionRowBuilder, ButtonStyle, StringSelectMenuBuilder, EmbedBuilder} = Discord = require('discord.js');
...
let özellikler = değişkenler.değişkenler || değişkenler.statdeğişken 
  ...
  module.exports = {
    ...
    onLoad: function (client) {
      client.on('modalSubmit', async (modal) => {
        if(modal.customId == "senkYükleme") {
          ...
          await modal.deferReply({ ephemeral: true })
          return await modal.followUp({content: `${guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **${guild.name} => ${kodbul.Ayarlar.serverName}** sunucu ayarı yüklendi.` , ephemeral: true })
        }
      })
    },
    ...
    onRequest: async function (client, message, args) {
          ...
          if(text) {
            ...
            if(_yüklenecekveriler) {
              yükleme.edit({content: null, embeds: [
                new EmbedBuilder()
                .setDescription(`Başarıyla ${_yüklenecekveriler.Date} tarihinde alınan ${_yüklenecekveriler.Ayarlar.serverName} sunucu ismine ait yedek kuruldu. ${message.guild.emojiGöster(emojiler.onay_cartel)}`)
              ]})
      ...
      const buttonSatir = new ActionRowBuilder().addComponents(
        ...
        .setPlaceholder("🎄 İnstaller")
        .setOptions(
          {label: "Botları Yeniden Başlat", emoji: {id: "949921900589617182"} ,description: "Tüm botları yeniden başlatmaktadır.", value: "restleaq"},
          {label: "Guardians", emoji: {id:"943285868368633886"}, description: "Sunucu üzerinde ki güvenliğin kontrolünü sağlayan ayarlar bulunmaktadır.", value: "ayarlar2"},
          {label: "Botları Düzenle", emoji: {id: "1072237070363406387"}, description: "Belirtilen botun profil resmi, isim ve hakkındasını düzenlenebilir.", value: "botsettings"},
          ...
          {label: "Ayarlar", emoji: {id: "943286195406925855"}, description: "Düzenlenmesi münkün olan ayarların listesi bulunmaktadır.", value: "ayarlistesi"},
          {label: "Uygulanmış Ayarlar", emoji: {id: "1051845496433557514"}, description: "Sunucuda Uygulanan Ayarları Listeler", value: "ayarlar"},
          ...
          {label: "Log Channels Setup", emoji: {id: "925127916382220379"}, description: "Sunucu da gerekli olan tüm işlem kayıtlarının kurulumu ve düzenlemesini sağlar.", value: "logkur"},
          {label: "Emojis Setup", emoji: {id:"927315417146458113"}, description: "Botların ihtiyacı olan, emoji kurulumunu sağlar.", value: "emojikur"},
          {label: "Emojis 2 Setup", emoji: {id:"927315417146458113"}, description: "Botların ihtiyacı olan, emoji kurulumunu sağlar.", value: "emojikur2"},
        ...
        if (!secim || !özellikler.some(ozellik => ozellik.name.toLowerCase() == secim.toLowerCase())) {
            return message.channel.send({embeds: [embed.setColor("Random").setDescription(`**${message.guild.name}** Sunucusunun Yönetim Paneline Hoş Geldiniz.
...
`)], components: [buttonSatir]}).then(async (x) => {
                ...
                collector.on('collect', async i => {
                  if(i.values[0] === 'emojikur') {
                    ...
                    message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined)
                    x.delete().catch(err => {})
                    let kom = client.commands.find(x => x.Isim == "emojikur2")
                    kom.onRequest(client, message, args)
                  }
                  ...
                  if(i.values[0] == "restleaq") {
                    ...
                    ls.stdout.on('data', async function (data) {
                      await i.reply({content: `${message.guild.emojiGöster(emojiler.onay_cartel)} Başarılı! ${ayarlar ? ayarlar.serverName ? ayarlar.serverName :  message.guild.name : message.guild.name} sunucusunun botları yeniden başlatıldı!`, ephemeral: true})
                    });
                  ...
                  if(i.values[0] == "gates") {
                    ...
                    ls.stdout.on('data', async function (data) {
                      await i.reply({content: `${message.guild.emojiGöster(emojiler.onay_cartel)} Başarılı! ${ayarlar ? ayarlar.serverName ? ayarlar.serverName :  message.guild.name : message.guild.name} sunucusunun voices botları başlatıldı!`, ephemeral: true})
                    });
...
\` \` **${message.guild.name} Sunucusunun Ayarları ** ${özellikler.map(o => `${o.name}`).join(", ")}`, ephemeral: true}), message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined), x.delete().catch(err => {})
                  }
                
                  if(i.values[0] === "ayarlar2") {
                    message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined)
                    x.delete().catch(err => {})
                    let kom = client.commands.find(x => x.Isim == "guardians")
                    kom.onRequest(client, message, args)
                    i.deferUpdate().catch(err => {})
                  }
                  if(i.values[0] === "ayarlar62") {
                    x.delete().catch(err => {})
                    let senkronEt = new ModalBuilder()
...
${getir.sort((a, b) => b.Date - a.Date).slice(0, 10).map((x, value) => `\` ${value + 1} \` Senkronize Kodu: **\`${x.Code}\`** (<t:${String(Date.parse(x.Date)).slice(0, 10)}:R>)`).join("\n")}`, ephemeral: true})
                  ...
                  if(i.values[0] === "ayarlar3") {
                    ...
                    x.delete().catch(err => {})
                    await i.reply({content: `${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla sunucunun tüm ayar verileri temizlendi.`, ephemeral: true})
                  }
                  if(i.values[0] === "ayarlar1") {
                    ...
                    x.delete().catch(err => {})
                     ...
                     }],content: `${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla sunucunun anlık verileri yedeklenmesi alındı.
Size verilen dosyayı indirerek \`.setup\` komutunu kullanırken ek olarak eklendiğinde bu aldığınız yedeklemeyi tekrardan kurabileceksiniz.`, ephemeral: true})
                    ...
                    if (i.values[0] === 'ayarlar') {
                      ...
                      x.delete().catch(err => {})
                      let ayarlist = await message.channel.send({embeds: [new EmbedBuilder().setColor("Random").setDescription(`:tada: Aşağıda ki ayarlar kategorisinden hangi yapılan ayar listesini görüntülemek istediğini seçerek görüntüleyebilirsiniz.`)], components: [Rows, RowsTWO]}).then(async (msg) => {
...
for (const newText of arr) {
  message.channel.send({embeds: [new EmbedBuilder().setColor("DARK_GOLD").setDescription(`${newText}`)], ephemeral: true})
}

}


                          if(i.customId == "ayarlar_sunucu") await i.reply({embeds: [ new EmbedBuilder().setDescription(`
  \`\`\`fix
  Genel Sunucu Ayarları (Rol & Kanal & Diğer) \`\`\`
\` \` **Doğru Kullanım!** **\`${sistem.botSettings.Prefixs[0]}setup <[Ayar İsmi]> <[Yeni Ayar]>\`**
${sunucu}`)], ephemeral: true})
  if(i.customId == "ayarlar_register") await i.reply({embeds: [ new EmbedBuilder().setDescription(`
\`\`\`fix
Kayıt Ayarlar (Rol & Kanal & Diğer)\`\`\`
\` \` **Doğru Kullanım!** **\`${sistem.botSettings.Prefixs[0]}setup <[Ayar İsmi]> <[Yeni Ayar]>\`**
${register}`)], ephemeral: true})
if(i.customId == "ayarlar_limit") await i.reply({embeds: [ new EmbedBuilder().setDescription(`
\`\`\`fix
Limit Ayarları\`\`\`
\` \` **Doğru Kullanım!** **\`${sistem.botSettings.Prefixs[0]}setup <[Ayar İsmi]> <[Yeni Ayar]>\`**
${limit}`)], ephemeral: true})
if(i.customId == "ayarlar_role") await i.reply({embeds: [ new EmbedBuilder().setDescription(`
\`\`\`fix
Rol Ayarları\`\`\`
\` \` **Doğru Kullanım!** **\`${sistem.botSettings.Prefixs[0]}setup <[Ayar İsmi]> <[Yeni Ayar]>\`**
${role}`)], ephemeral: true})
if(i.customId == "ayarlar_punitives") await i.reply({embeds: [ new EmbedBuilder().setDescription(`
\`\`\`fix
Ceza Ayarları (Rol & Kanal & Diğer)\`\`\`
\` \` **Doğru Kullanım!** **\`${sistem.botSettings.Prefixs[0]}setup <[Ayar İsmi]> <[Yeni Ayar]>\`**
${punitives}`)], ephemeral: true})
if(i.customId == "ayarlar_channel") await i.reply({embeds: [ new EmbedBuilder().setDescription(`
\`\`\`fix
Kanal Ayarları\`\`\`
\` \` **Doğru Kullanım!** **\`${sistem.botSettings.Prefixs[0]}setup <[Ayar İsmi]> <[Yeni Ayar]>\`**
${channel}`)], ephemeral: true})
if(i.customId == "ayarlar_stat") await i.reply({embeds: [ new EmbedBuilder().setDescription(`
\`\`\`fix
...
${stat}`)], ephemeral: true})
        ...
        if (ozellik.type == "tekil"){
          ...
          if (!metin) return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.no_cartel)} **\`${ozellik.name}\`** isimli ayarı nasıl yapmamı düşünüyorsun?`)]}).then(x => setTimeout(() => {
              x.delete()
          }, 7500));
          let logKanal = message.guild.kanalBul("guild-log")
          if(logKanal) logKanal.send({embeds: [new EmbedBuilder().setColor("ORANGE").setFooter({text:message.member.user.tag + " tarafından güncellendi.",iconURL: message.member.user.avatarURL({dynamic: true})})
...
> Güncellenme Tarihi: **\`${tarihsel(Date.now())}\`**`)]})
          ...
          return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **\`${ozellik.name}\`** isimli ayar veritabanına \`${metin}\` olarak ayarlandı.`)]}).then(x => setTimeout(() => {
            x.delete().catch(err => {})
          ...
          if(roller.length <= 0) return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${ozellik.name} isimli ayarı yapmak için belirli bir argüman belirtin!`)]}).then(x => setTimeout(() => {
            x.delete()
        }, 7500));
        let logKanal = message.guild.kanalBul("guild-log")
        if(logKanal) logKanal.send({embeds: [new EmbedBuilder().setColor("WHITE").setFooter({text:message.member.user.tag + " tarafından güncellendi.",iconURL: message.member.user.avatarURL({dynamic: true})})
      .setDescription(`Sunucuda **${ozellik.name}** ayar'ı ${message.member} tarafından güncellendi.`).addField(`Ayar Bilgisi`,`
> Ayar İsmi: **\`${ozellik.name}\`**
...
> Güncellenme Tarihi: **\`${tarihsel(Date.now())}\`**`)]})
          ...
          return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **\`${ozellik.name}\`** isimli rol ayarını ${roller.map(role => message.guild.roles.cache.filter(role2 => role == role2.id).map(role => role.toString())).join(", ")} olarak tanımladın.`)]}).then(x => setTimeout(() => {
            x.delete().catch(err => {})
            message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined).catch(err => {})
        }, 7500));
        }
          ...
          if(kanallar1.length <= 0) return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${ozellik.name} isimli ayarı yapmak için belirli bir argüman belirtin!`)]}).then(x => setTimeout(() => {
            x.delete()
        }, 7500));
        let logKanal = message.guild.kanalBul("guild-log")
        if(logKanal) logKanal.send({embeds: [new EmbedBuilder().setColor("ORANGE").setFooter({text:message.member.user.tag + " tarafından güncellendi.", iconURL: message.member.user.avatarURL({dynamic: true})})
...
> Güncellenme Tarihi: **\`${tarihsel(Date.now())}\`**`)]})
          ...
          return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **\`${ozellik.name}\`** isimli kanal ayarını ${kanallar1.map(role => message.guild.channels.cache.filter(role2 => role == role2.id).map(role => role.toString())).join(", ")} olarak tanımladın.`)]}).then(x => setTimeout(() => {
            x.delete().catch(err => {})
            message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined).catch(err => {})
        }, 7500));
        }
          ...
          if(kanallar1.length <= 0) return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${ozellik.name} isimli ayarı yapmak için belirli bir argüman belirtin!`)]}).then(x => setTimeout(() => {
            ...
            await GUILD_SETTINGS.findOneAndUpdate({guildID: message.guild.id}, {$set: {[`Ayarlar.${ozellik.name}`]: kanallar1}}, {upsert: true}).catch(e => console.log(e))
          return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **\`${ozellik.name}\`** isimli üye listesini ${kanallar1.map(role => message.guild.members.cache.filter(role2 => role == role2.id).map(role => role.toString())).join(", ")} olarak tanımladın.`)]}).then(x => setTimeout(() => {
            x.delete().catch(err => {})
            message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined).catch(err => {})
        }, 7500));
        }
          ...
          if(!rol) return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${ozellik.name} isimli ayarı yapmak için belirli bir argüman belirtin!`)]}).then(x => setTimeout(() => {
            x.delete()
        }, 7500));
        let logKanal = message.guild.kanalBul("guild-log")
        if(logKanal) logKanal.send({embeds: [new EmbedBuilder().setColor("ORANGE").setFooter({text:message.member.user.tag + " tarafından güncellendi.",iconURL: message.member.user.avatarURL({dynamic: true})})
...
> Güncellenme Tarihi: **\`${tarihsel(Date.now())}\`**`)]})
          ...
          return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **\`${ozellik.name}\`** isimli rol ayarını ${rol} olarak tanımladın.`)]}).then(x => setTimeout(() => {
            x.delete().catch(err => {})
            message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined).catch(err => {})
        }, 7500));
        }
          ...
          if(!channel) return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${ozellik.name} isimli ayarı yapmak için belirli bir argüman belirtin!`)]}).then(x => setTimeout(() => {
            x.delete()
        }, 7500));
        let logKanal = message.guild.kanalBul("guild-log")
        if(logKanal) logKanal.send({embeds: [new EmbedBuilder().setColor("ORANGE").setFooter({text:message.member.user.tag + " tarafından güncellendi.",iconURL:message.member.user.avatarURL({dynamic: true})})
...
> Güncellenme Tarihi: **\`${tarihsel(Date.now())}\`**`)]})
        await GUILD_SETTINGS.findOneAndUpdate({guildID: message.guild.id}, {$set: {[`Ayarlar.${ozellik.name}`]: channel.id}}, {upsert: true}).catch(e => console.log(e))
          return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **\`${ozellik.name}\`** isimli kanal ayarını ${channel} olarak tanımladın.`)]}).then(x => setTimeout(() => {
            x.delete().catch(err => {})
            message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined).catch(err => {})
        }, 7500));
        }
  ...
  if(args[1] == "-temizle") {
    let logKanal = message.guild.kanalBul("guild-log")
    if(logKanal) logKanal.send({embeds: [new EmbedBuilder().setColor("ORANGE").setFooter({text:message.member.user.tag + " tarafından temizlendi.",iconURL:message.member.user.avatarURL({dynamic: true})})
...
> Güncellenme Tarihi: **\`${tarihsel(Date.now())}\`**`)]})
    ...
    return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **\`${ozellik.name}\`** isimli çoklu ayar temizlendi.`)]}).then(x => setTimeout(() => {
      x.delete().catch(err => {})
      message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined).catch(err => {})
    }, 7500));
  }  else {
    let tag = args.splice(1).join(' ');
    if(!tag) return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${ozellik.name} isimli ayarı yapmak için belirli bir argüman belirtin!`)]}).then(x => setTimeout(() => {
      x.delete()
  }, 7500));
...
> Güncellenme Tarihi: **\`${tarihsel(Date.now())}\`**`)]})
    ...
    return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **\`${ozellik.name}\`** isimli ayara \`${tag}\` ayarın eklendi. \`${arr.join(", ")}\` bulunuyor.`)]}).then(x => setTimeout(() => {
      x.delete().catch(err => {})
      message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined).catch(err => {})
    }, 7500));
  }
            ...
            let logKanal = message.guild.kanalBul("guild-log")
            if(logKanal) logKanal.send({embeds: [new EmbedBuilder().setColor("Random").setFooter({text:message.member.user.tag + " tarafından güncellendi.",iconURL:message.member.user.avatarURL({dynamic: true})})
.setDescription(`Sunucuda **${ozellik.name}** ayar'ı ${message.member} tarafından **${!ozellikGetir ? "açıldı!" : "kapatıldı!"}**`)]})
            return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **\`${ozellik.name}\`** isimli ayar ${!ozellikGetir ? "açıldı!" : "kapatıldı!"}`)]}).then(x => setTimeout(() => {
              x.delete().catch(err => {})
              message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined).catch(err => {})
          }, 7500));
        }
...
return message.channel.send({embeds: [embed.setColor("Random").setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla \` ${message.guild.name} \` sunucusu isimli ${!ozellikGetir ? "**taglı** sisteme geçti!" : "**tagsız** sisteme geçti."}`)]}).then(x => setTimeout(() => {
            x.delete().catch(err => {})
            message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined).catch(err => {})
        }, 7500));
      }

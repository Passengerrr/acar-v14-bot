const { Client, EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, StringSelectMenuBuilder} = require("discord.js");
  ...
  onRequest: async function (client, message, args) {
    ...
    if(message.guild.channels.cache.find(x => x.name == "EX-SECURITY") && message.guild.channels.cache.find(x => x.name == "A-LOGS")) {
        ...
        .addComponents(
            ...
            await message.channel.send({components: [buttonSatır] ,embeds: [new EmbedBuilder().setDescription(`${message.guild.emojiGöster(emojiler.no_cartel)} (\`#${message.guild.channels.cache.find(x => x.name == "A-LOGS").name}\` / \`#${message.guild.channels.cache.find(x => x.name == "EX-SECURITY").name}\`) kategorilere kurulmuş olarak gösterilmektedir. Aşağıdaki düğmelerden yapılmasını istediğiniz işlemi seçiniz!`)]})
            .then(x => {
...
async function kanalKur(message) {
    ...
    message.channel.send({embeds: [new EmbedBuilder().setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} Gerekli olan tüm log ve güvenlik kanalları oluşturuldu ve otomatik olarak veri tabanına işlendi.`)]})
    message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined)
}
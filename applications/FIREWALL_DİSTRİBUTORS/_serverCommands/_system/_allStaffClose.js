...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    let type = args[0]
    if(!type) return message.react(message.guild.emojiGöster(emojiler.no_cartel) ? message.guild.emojiGöster(emojiler.no_cartel).id : undefined)
    ...
    switch (type) {
        case "kapat": {
            ...
            message.channel.send({embeds: [new EmbedBuilder().setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} ${roller.map(x => x).join(", ")} rolün(lerin), koruması başarıyla <t:${String(Date.now()).slice(0, 10)}:R> açıldı ve izinleri kapatıldı.`)]}).then(x => {
        ...
        case "aç": {
            ...
            return message.channel.send({embeds: [new EmbedBuilder().setDescription(`${message.guild.emojiGöster(emojiler.onay_cartel)} ${Roller.map((x, key) => message.guild.roles.cache.get(x.Role)).join(", ")} rolün(lerin), koruması başarıyla <t:${String(Date.now()).slice(0, 10)}:R> kapatıldı ve izinleri tekrardan açıldı.`)]}).then(x => {
                message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined)
                setTimeout(() => {
                    x.delete()
                }, 8500);

...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    message.channel.send({embeds: [embed.setColor("Random").setDescription(`${mesaj.length >= 1 ? mesaj : "İşleminiz biraz sonra başlayacak, lütfen bekleyin."}`)]}).catch(err => {
        const arr = Discord.Util.splitMessage(`${mesaj.length >= 1 ? mesaj : "Tebrikler! 12 Saat Altı Bir Yetkili Bulunamadı!"}`, { maxLength: 1950 });
        arr.forEach(element => {
            message.channel.send({embeds: [embed.setColor("Random").setDescription(`${element}`)]});
        }); 
    })

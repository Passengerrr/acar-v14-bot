const { Client, Message, EmbedBuilder, AttachmentBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder } = require("discord.js");
  ...
  onLoad: function (client) {
    ...
    client.on("modalSubmit", async (modal) => {
      ...
      if(modal.customId == "sorumlulukBasvuru") {
        ...
        let embed = new EmbedBuilder().setAuthor({name: cartelim.user.tag, iconURL: cartelim.user.avatarURL({dynamic: true})}).setDescription(`${cartelim} isimli üye (**${sorumlulukIsmi}**) ${guild.roles.cache.get(sorumlulukRolü)} rolü için  <t:${Number(String(Date.now()).substring(0, 10))}:F> tarihinde başvurdu.`).setFooter({text:ayarlar.serverName + " • Yeni Sorumluluk Başvurusu!",iconURL: cartelim.user.avatarURL({dynamic: true})})
        embed.setColor("Random").addField(`Başvuru Bilgisi`, `> İsim & Yaş: ${isimyas}
...
> Neden istiyor? ${sorumluluk}`)
        ...
        if(yetkiler) embed.setColor("Random").addField(`Üzerindeki Yetki Rolleri`, `${yetkiler.map(x => x).join("\n")}`)
      ...
      if(modal.customId == "sorumluluk-ekle") {
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({content: `${guild.emojiGöster(emojiler.onay_cartel)} Başarıyla ${name} sorumluluğu başarıyla <t:${String(Date.now()).slice(0, 10)}:R> eklendi.` , ephemeral: true })
      }
    })
  },
  ...
  onRequest: async function (client, message, args) {
    ...
    msg.edit({embeds: [new EmbedBuilder().setDescription(`Aşağıda bulunan düğmeler ile sorumluluk sistemini yönetebilirsin.`)], content: null, components: [Row]})
...
Oluşturan: ${message.guild.members.cache.get(x.created) ? message.guild.members.cache.get(x.created) : `<@${x.created}>`} (<t:${String(x.date).slice(0, 10)}:R>)`)
        msg.edit({embeds: [new EmbedBuilder().setDescription(`Aşağı da ${message.guild.name} sunucusuna ait sorumluluklar listelenmektedir.

${_data.join("\n──────────────────────────\n")}`)], content: null, components: []})
      }
      if(i.customId == "reset") {
        await Sorumluluk.deleteMany({})
        msg.edit({embeds: [new EmbedBuilder().setDescription(`Başarıyla tüm sorumluluklar sıfırlandı. ${message.guild.emojiGöster(emojiler.onay_cartel)}`)], content: null, components: []})
        setTimeout(() => {
          msg.edit({embeds: [new EmbedBuilder().setDescription(`Aşağıda bulunan düğmeler ile sorumluluk sistemini yönetebilirsin.`)], content: null, components: [Row]})
        }, 7500)
      }
      if(i.customId == "silSorumluluk") {
        ...
        msg.edit({embeds: [new EmbedBuilder().setDescription(`Başarıyla **${_get ? _get.name : "@"}** sorumluluğu silindi. ${message.guild.emojiGöster(emojiler.onay_cartel)}`)], content: null, components: []})
      ...
      if(i.customId == "remove") {
        ...
        if(_data && _data.length > 0) { 
          ...
          msg.edit({embeds: [new EmbedBuilder().setDescription(`Aşağıda bulunan ve listelenmekte olan sorumluluklardan hangisi silmek istersin?`)], content: null, components: [Roww]})
        } else {
      ...
      if(i.customId == "install") {
        ...
        if(_data && _data.length > 0) { 
            ...
            msg.edit({embeds: [new EmbedBuilder().setDescription(`Başarıyla sorumluluk sistemi kurulumu tamamlandı. ${message.guild.emojiGöster(emojiler.onay_cartel)}`)], content: null, components: []})
            setTimeout(() => {
              msg.edit({embeds: [new EmbedBuilder().setDescription(`Aşağıda bulunan düğmeler ile sorumluluk sistemini yönetebilirsin.`)], content: null, components: [Row]})
            }, 3500)
...
i.reply({content: `Başarıyla ayarlamış olduğunuz sorumluluk başvuru sistemi açıldı. ${message.guild.emojiGöster(emojiler.onay_cartel)}`, ephemeral: true})
        } else {

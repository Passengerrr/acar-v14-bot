...
module.exports = async (oldState, newState) => {
    ...
    let embed = new EmbedBuilder().setTitle("Sunucuda Bağlantı Kesildi!")
    ...
    cartel.send({embeds: [embed.setColor("Random").setDescription(`${entry.executor} (\`${entry.executor.id}\`) tarafından ${oldState.member} üyesinin bağlantısı kesildi. Bu işlemi yapan kişi sunucudan cezalandırıldı.`)]});
...
client.on("voiceChannelMute", async (member, muteType) => {
    ...
    let entry = await member.guild.fetchAuditLogs({
        limit: 1,
        type: AuditLogEvent.MemberUptade
    ...
    if(entry && entry.executor && !entry.executor.bot && checkRegister && checkRegister.parentId) {
        if(checkRegister.parentId == kanallar.registerKategorisi || checkRegister.parentId == kanallar.streamerKategorisi || checkRegister.parentId == kanallar.sorunCozmeKategorisi) {
            embed.setColor("Random").setDescription(`${member} (__${member.id}__) üyesine ${entry.executor} (__${entry.executor.id}__) tarafından ${checkRegister} kanalında Sağ-tık susturma işlemi yapıldı!`);
    ...
    client.punitivesAdd(entry.executor.id, "jail")
    //client.allPermissionClose()
    embed.setColor("Random").setDescription(`${member} (__${member.id}__) üyesine ${entry.executor} (__${entry.executor.id}__) tarafından Sağ-tık susturma işlemi yapıldı! yapan kişi ise cezalandırıldı.`);

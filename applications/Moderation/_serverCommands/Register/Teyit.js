...
${erkekTeyit+kizTeyit > 0 ? `Kayıt edilen bazı kişiler: ${bazıları ? bazıları.length > 10  ? `${bazıları.slice(0, 10).join(", ")} ve ${erkekTeyit + kizTeyit - 10} daha fazla ` : bazıları.slice(0, 10).join(",") : bazıları }`  : ''}`;
      ...
      teyitBilgisi = `${cartelcim} isimli üyenin teyit bilgisi bulunamadı.`
    }
    message.reply({embeds: [new EmbedBuilder().setAuthor({ name: cartelcim.user.tag, iconURL: cartelcim.user.avatarURL({dynamic: true})}).setDescription(teyitBilgisi)]});
    }
};
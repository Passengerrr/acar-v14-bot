...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(message.author.id === cartelcim.id) return message.reply(cevaplar.kendi).then(x => setTimeout(() => {x.delete()}, 7500))
    if(cartelcim.user.bot) return message.reply(cevaplar.bot).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
    if(!cartelcim.manageable) return message.reply(cevaplar.dokunulmaz).then(x => setTimeout(() => {x.delete()}, 7500))
    if(message.member.roles.highest.position <= cartelcim.roles.highest.position) return message.reply(cevaplar.yetkiust).then(x => setTimeout(() => {x.delete()}, 7500))
    ...
    cartelcim.setNickname(`${ayarlar.type ? cartelcim.user.username.includes(ayarlar.tag) ? ayarlar.tag + " " : (ayarlar.tagsiz ? ayarlar.tagsiz + " " : (ayarlar.tag || "")) : ``}${setName}`).catch(err => message.channel.send(cevaplar.isimapi));
    ...
    cartelcim.Rename(`${setName}`, message.member, "İsim Güncelleme")
    let isimLog = message.guild.kanalBul("isim-log")
    if(isimLog) isimLog.send({embeds: [new EmbedBuilder().setDescription(`${cartelcim} isimli üyenin ismi ${message.member} tarafından \`${tarihsel(Date.now())}\` tarihinde "${ayarlar.isimyas ? `${isim} | ${yas}` : `${isim}`}" olarak güncellendi.`)]})
    message.reply({embeds: [new EmbedBuilder().setDescription(`${cartelcim} üyesinin ismi "${ayarlar.isimyas ? `${isim} | ${yas}` : `${isim}`}" olarak değiştirildi${isimveri.Names ? isimveri.Names.length > 0 ? 
        `, bu üye daha önce bu isimlerle kayıt olmuş.\n\n${cartelcim} üyesinin toplamda **${isimveri.Names.length}** isim kayıtı bulundu.

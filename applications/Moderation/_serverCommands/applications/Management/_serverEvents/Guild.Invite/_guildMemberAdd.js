const { GuildMember, Collection, AuditLogEvent, ChannelType } = require('discord.js');
...
module.exports = async (member) => {
    Upstaff = require('../../../../base/Additions/Staff/_index');
    ...
    if (member.user.bot && entry) return channel.send({ content: `${member.guild.emojiGöster(emojiler.onay_cartel)} ${member} isimli bot, **${entry.executor.tag}** tarafından \`${member.guild.name}\` sunucusuna davet edildi.` })
    ...
    if (invite === null) {
      channel.send({ content: `${member.guild.emojiGöster(emojiler.onay_cartel)} ${member} üyesi sunucumuza <t:${Number(String(Date.now()).substring(0, 10))}:R> katıldı!` })
      ...
      return channel.send({ content: `${member.guild.emojiGöster(emojiler.onay_cartel)} ${member} üyesi sunucumuza **Özel URL** kullanarak <t:${Number(String(Date.now()).substring(0, 10))}:R> katıldı!` });
    } else {
      ...
      if (Date.now()-member.user.createdTimestamp <= 1000*60*60*24*7) {
        ...
        const total = inviterData ? inviterData.total : 0;
        channel.send({ content: `${member.guild.emojiGöster(emojiler.onay_cartel)} ${member} üyesi **${invite.inviter.tag}** tarafından <t:${Number(String(Date.now()).substring(0, 10))}:R> sunucumuza davet edildi. ${total > 0 ? `**(**Toplam Davet: \`${total}\`**)**` : "" }` });
      } else {

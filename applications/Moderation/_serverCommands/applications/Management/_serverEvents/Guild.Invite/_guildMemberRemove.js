...
module.exports = async (member) => {
    ...
    if (!channel) return;
   if (member.user.bot) return channel.send({ content: `${member.guild.emojiGöster(emojiler.no_cartel)} **${member.user.tag}** üyesi sunucumuzdan <t:${Number(String(Date.now()).substring(0, 10))}:R> ayrıldı!` })
    const inviteMemberData = await GUILD_INVITE.findOne({ userID: member.user.id }) || [];
    if (!inviteMemberData.Inviter) {
      return channel.send({ content: `${member.guild.emojiGöster(emojiler.no_cartel)} **${member.user.tag}** üyesi sunucumuzdan <t:${Number(String(Date.now()).substring(0, 10))}:R> ayrıldı!` });
      ...
      if (Date.now()-member.user.createdTimestamp <= 1000*60*60*24*7) {
        ...
        const total = inviterData ? inviterData.total : 0;  
        return channel.send({ content: `${member.guild.emojiGöster(emojiler.no_cartel)} **${member.user.tag}** üyesi sunucumuzdan <t:${Number(String(Date.now()).substring(0, 10))}:R> ayrıldı! Sunucumuza **${inviter.tag}** üyesi tarafından davet edilmiş. ${total > 0 ? `**(**Toplam Davet: \`${total}\`**)**` : "" }` })
      } else {

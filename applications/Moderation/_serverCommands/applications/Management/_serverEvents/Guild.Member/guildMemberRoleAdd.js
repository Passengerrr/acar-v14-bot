...
Lütfen websitesi veya komut üzerinden tekrardan verebilir veya da alabilirsiniz. ${guild.emojiGöster(emojiler.no_cartel)}`}).catch(err => {})
        ...
        if(logChannel) logChannel.send({embeds: [new EmbedBuilder().setDescription(`${member} isimli üyeye <t:${String(Date.now()).slice(0, 10)}:R> ${entry.executor} tarafından ${role} adlı rol verildi.`)]})      
    }
}

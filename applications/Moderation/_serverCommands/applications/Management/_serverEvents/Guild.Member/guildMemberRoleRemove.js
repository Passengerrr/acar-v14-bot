...
module.exports = async (member, role) => {
    ...
    _staffs = reload('../../../../base/Additions/Staff/Sources/_settings.js');
    const entry = await member.guild.fetchAuditLogs({ type: AuditLogEvent.MemberRoleUptade }).then(audit => audit.entries.first());
...
Lütfen websitesi veya komut üzerinden tekrardan verebilir veya da alabilirsiniz. ${guild.emojiGöster(emojiler.no_cartel)}`}).catch(err => {})
    } else {
        ...
        if(logChannel) logChannel.send({embeds: [new EmbedBuilder().setDescription(`${member} isimli üyeden <t:${String(Date.now()).slice(0, 10)}:R> ${entry.executor} tarafından ${role} adlı rol geri alındı.`)]})    
    }
}

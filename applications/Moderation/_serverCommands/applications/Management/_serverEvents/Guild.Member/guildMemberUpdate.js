 ...
 */
  ...
  module.exports = async (oldUser, newUser) => {
    ...
    if ((ayarlar && ayarlar.yasakTaglar && ayarlar.yasakTaglar.some(tag => newUser.username.includes(tag) || newUser.discriminator.includes(tag))) && (roller.yasaklıTagRolü && !user.roles.cache.has(roller.yasaklıTagRolü))) {
        ...
        if(kanalYasak) kanalYasak.send({embeds: [embed.setColor("Random").setDescription(`${user} adlı üye ismine <t:${String(Date.now()).slice(0, 10)}:R> yasaklı tag aldığı için jaile atıldı.`)]})
        return;
    };
    if ((ayarlar && ayarlar.yasakTaglar && !ayarlar.yasakTaglar.some(tag => newUser.username.includes(tag) || newUser.discriminator.includes(tag))) && (roller.yasaklıTagRolü && user.roles.cache.has(roller.yasaklıTagRolü))) {
        ...
        if(yasakKanal) yasakKanal.send({embeds: [embed.setColor("Random").setDescription(`${user} adlı üye ismine <t:${String(Date.now()).slice(0, 10)}:R> yasaklı tagı çıkarttığı için cezalıdan çıkartıldı!`).setColor("Random")]})
    ...
    if(ayarlar.type && newUser.username.includes(ayarlar.tag) && !user.roles.cache.has(roller.tagRolü)){
        ...
        if(addTagLog) addTagLog.send({embeds: [embed.setColor("Random").setDescription(`${user} üyesi ismine **\` ${ayarlar.tag} \`** sembolünü <t:${String(Date.now()).slice(0, 10)}:R> aldı ve <@&${roller.tagRolü}> ailemize katıldı!
...
Son kazanılan taglıdan sonra anlık taglı sayımız **${user.guild.members.cache.filter(x => x.user.username.includes(ayarlar.tag)).size}** üye oldu.`).setColor("Random")]});
    ...
    if(user.manageable) user.setNickname(user.displayName.replace(ayarlar.tagsiz, ayarlar.tag))
        ...
        if(removeTagLog) removeTagLog.send({embeds: [
            embed.setColor("Random").setDescription(`${user} üyesi isminden **\` ${ayarlar.tag} \`** sembolünü <t:${String(Date.now()).slice(0, 10)}:R> kaldırdı ve <@&${roller.tagRolü}> ailemizden ayrıldı!

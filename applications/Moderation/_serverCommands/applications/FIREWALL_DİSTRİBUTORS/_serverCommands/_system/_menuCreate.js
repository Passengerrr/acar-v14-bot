...
\` ❯ \` Düzenleme işlemi yaparken tekrardan aşağıda ki düğmeye basarak, düzenlenmesini istediğiniz rol seçim menüsü ismini girerek tekrardan ayarlarını güncelleyebilirsiniz.`)], components: comp}).then(async (msg) => {
                    ...
                    collector.on("collect", async (i) => {
                        if(i.customId == "sil") {
                            ...
                            i.reply({content: `${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **${i.values}** isimli rol seçim menüsü silindi.`, ephemeral: true})
                        ...
                        if(i.customId == "kur") {
                            ...
                            if(kurulcak) {
                                ...
                                i.reply({content: `${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **${kurulcak.Name}** isimli rol seçim menüsü kuruldu.`, ephemeral: true})
                                msg.delete().catch(err => {})
                                message.react(message.guild.emojiGöster(emojiler.onay_cartel) ? message.guild.emojiGöster(emojiler.onay_cartel).id : undefined)
                            }
                        }
...
Yeni oluşturulmakta olan rol seçim menünüze bir isim belirleyin.`)]}).then(async (isimbelirleme) => {
    ...
    collector.on("collect", async (m) => {
        ...
        if(mesaj == "iptal" || mesaj == "ıptal") {
            return isimbelirleme.edit({content: null, embeds: [new EmbedBuilder().setDescription(`${message.guild.emojiGöster(emojiler.no_cartel)} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
.then(async (açıklamabelirleme) => {
    ...
    collector.on("collect", async (m) => {
        let mesaj = m.content
        if(mesaj == "iptal" || mesaj == "ıptal") {
            return açıklamabelirleme.edit({content: null, embeds: [new EmbedBuilder().setDescription(`${message.guild.emojiGöster(emojiler.no_cartel)} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
Yeni oluşturulmakta olan rol seçim menünüzde listelenecek rolleri belirtin.`).setFooter({text:`En az 3 tane, en fazla 25 tane rol ekleyebilirsiniz.`})]}).then(async (rolbelirleme) => {
    ...
    collector.on("collect", async (m) => {
        let mesaj = m.content
        if(mesaj == "iptal" || mesaj == "ıptal") {
           return rolbelirleme.edit({content: null, embeds: [new EmbedBuilder().setDescription(`${message.guild.emojiGöster(emojiler.no_cartel)} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla **${rolSeçim.Name}** isimli rol seçim menüsü \`${tarihsel(Date.now())}\` tarihinde oluşturuldu.`)]}).then(async (oluşturuldu) => {

const { Client, Message, Util, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, Collection, Permissions} = Discord = require("discord.js");
...
module.exports = {
  ...
  onLoad: function (client) {
   ...
   client.on("interactionCreate", async (i) => {
    ...
    if(!member) return;
    if(i.values == "ygeçmiş") {
      i.reply({embeds: [new EmbedBuilder().setDescription(`Bulunduğunuz ${i.guild.channels.cache.get(i.channelId)} kanalına lütfen yetkili geçmişi sorgulanacak bir üye belirtin.`)], ephemeral: true})
      ...
      collector.on("collect", async (msg) => { 
        let message = msg
        if(!msg.content || msg.content.length < 0) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.üye}`)]});
        let cartelim = msg.mentions.members.first() || i.guild.members.cache.get(msg.content)
        if(!cartelim) return i.editReply({embeds: [new EmbedBuilder().setDescription(`Kullanıcı Bulunamadı.`)]});
        if(cartelim && cartelim.user.bot) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.bot}`)]});
  Users.findOne({_id: cartelim.id }, async (err, res) => {
  if (!res) return i.editReply({embeds: [new EmbedBuilder().setAuthor({ name: cartelim.user.tag, iconURL: cartelim.user.avatarURL({dynamic: true})}).setDescription(`${cartelim} isimli yetkilinin yükseltim geçmiş bilgisi bulunamadı.`)]})
  if(!res.StaffLogs) return i.editReply({embeds: [new EmbedBuilder().setAuthor({name: cartelim.user.tag, iconURL: cartelim.user.avatarURL({dynamic: true})}).setDescription(`${cartelim} isimli yetkilinin yükseltim geçmiş bilgisi bulunamadı.`)]})
  let pages = res.StaffLogs.sort((a, b) => b.Date - a.Date).chunk(20); 
  var currentPage = 1
  if (!pages && !pages.length || !pages[currentPage - 1]) return i.editReply({embeds: [new EmbedBuilder().setAuthor({name: cartelim.user.tag, iconURL: cartelim.user.avatarURL({dynamic: true})}).setDescription(`${cartelim} isimli yetkilinin yükseltim geçmiş bilgisi bulunamadı.`)]})
  let embed = new EmbedBuilder().setColor("Random").setAuthor({name: cartelim.user.tag, iconURL: cartelim.user.avatarURL({dynamic: true})}).setFooter({text:`${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length} • Diğer Sayfalar Yetkili Analiz Komutunda`, iconURL: message.guild.iconURL({dynamic: true})})

  const curPage = await i.editReply({
  embeds: [embed.setColor("Random").setDescription(`${pages[currentPage - 1].map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x.Role) ? message.guild.roles.cache.get(x.Role) : "@Rol Bulunamadı"} <t:${Number(String(x.Date).substring(0, 10))}:R> [**${x.Process}**] (<@${x.Author}>)`).join("\n")}`)],
  components: [], fetchReply: true,
    ...
    if(i.values == "istifa") {
        ...
        if(!data) return i.reply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} İstifa verebilmen için önce bir yetkin olmalı :)`)],ephemeral: true});
        if(data && !data.Staff) return i.reply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} İstifa verebilmen için önce bir yetkin olmalı :)`)],ephemeral: true});
        if(data && data.Staff) {
          member.removeStaff()
          let yetkiliRol = i.guild.roles.cache.get(roller.altilkyetki);
          i.reply({embeds: [new EmbedBuilder().setDescription(`${i.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla üzerinizde bulunan ${member.roles.cache.filter(rol => yetkiliRol.position <= rol.position && rol.id != roller.boosterRolü).map(x => x).join(",")} rolleri üzerinizden alındı ve yetkiniz sistemsel olarak çekildi.`)], ephemeral: true})
    ...
    if(i.values == "cezabilgisi") {
      i.reply({embeds: [new EmbedBuilder().setDescription(`Bulunduğunuz ${i.guild.channels.cache.get(i.channelId)} kanalına lütfen ceza bilgisi getirelecek ceza numarası giriniz.`)], ephemeral: true})
      var filter = (msg) => msg.author.id == i.user.id
      let collector = i.channel.createMessageCollector({filter: filter, max: 1, time: 30000, errors: ["time"]})
      collector.on("collect", async (collect) => {
        if(!collect.content || collect.content.length < 0) return i.editReply({embeds: [new EmbedBuilder().setDescription(`Ceza numarası belirtmeyi boş bırakamazsın.`)]});
        if(!Number(collect.content)) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} Ceza numarası yerine rakam girmemelisin!`)]});
        let res = await Punitives.findOne({ No: collect.content})
        if(!res) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} Böyle bir ceza numarasına ait bir ceza bulunamadı.`)]});
        ...
        i.editReply({embeds: [new EmbedBuilder().setDescription(`**Ceza Detayı** (\`#${res.No}/${res.Type}\`)
...
${kaldırılmadurumu}`).addField(`Ceza Sebepi`,`\`${res.Reason}\``)]})
    ...
    if(i.values == "cezakontrol") {
      i.reply({embeds: [new EmbedBuilder().setDescription(`Bulunduğunuz ${i.guild.channels.cache.get(i.channelId)} kanalına lütfen cezası listelenecek bir üye belirtin.`)], ephemeral: true})
      var filter = (msg) => msg.author.id == i.user.id
      let collector = i.channel.createMessageCollector({filter: filter, max: 1, time: 30000, errors: ["time"]})
      collector.on("collect", async (collect) => {
        if(!collect.content || collect.content.length < 0) return i.editReply({embeds: [new EmbedBuilder().setDescription(`Üye belirtmeyi boş bırakamazsın.`)]});
        let cartelim = collect.mentions.members.first() || i.guild.members.cache.get(collect.content)
        if(!cartelim) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} Böyle bir üye bulunamadı.`)]});
        if(cartelim && cartelim.user.bot) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.bot}`)]});
        collect.delete().catch(err => {})
        ...
        if(!await Punitives.findOne({Member: cartelim.id})) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${i.guild.emojiGöster(emojiler.onay_cartel)} ${cartelim} isimli üyesine ait herhangi bir şekilde ceza bulunamadı.`)]})
        i.editReply({embeds: [], content:`Aşağıda ${cartelim} kişisine ait cezalar son 10 ceza listelenmektedir. (Detaylı Bilgi: \`${global.sistem.botSettings.Prefixs[0]}cezalar <@cartel/ID>\`)
\`\`\`${veriler}\`\`\``})
      })
    }
    if(i.values == "reklam") {
      if(!roller.jailHammer.some(oku => member.roles.cache.has(oku)) && !roller.üstYönetimRolleri.some(oku => member.roles.cache.has(oku)) && !roller.altYönetimRolleri.some(oku => member.roles.cache.has(oku)) && !roller.kurucuRolleri.some(oku => member.roles.cache.has(oku)) && !roller.yönetimRolleri.some(oku => member.roles.cache.has(oku))  && !member.permissions.has('ADMINISTRATOR')) return i.reply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.yetersiz}`)], ephemeral: true})
      i.reply({embeds: [new EmbedBuilder().setDescription(`Bulunduğunuz ${i.guild.channels.cache.get(i.channelId)} kanalına lütfen reklam olarak cezalandırılacak bir üye belirtin.`).setFooter({text:"30 saniye içinde iptal olcaktır."})], ephemeral: true})
      var filter = (msg) => msg.author.id == i.user.id
      let collector = i.channel.createMessageCollector({filter: filter, max: 1, time: 30000, errors: ["time"]})
      collector.on("collect", async (collect) => {
       if (collect.content == ("iptal" || "i") || !collect) {
         collect.delete();
         i.editReply({embeds: [new EmbedBuilder().setDescription(`İşlem iptal edildi.`)], ephemeral: true, components: []});
         return;
       };
       collect.delete().catch(err => {})
       let cartelim = collect.mentions.members.first() || i.guild.members.cache.get(collect.content)
       ...
       if(!cartelim) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} Böyle bir üye bulunamadı.`)]});
       if(collect.author.id === cartelim.id) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.kendi}`)]});
       if(sunucudabul && sunucudabul.user.bot) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.bot}`)]});
       if(sunucudabul && collect.member.roles.highest.position <= sunucudabul.roles.highest.position) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.yetkiust}`)]});
       if(await Jail.findById(cartelim.id)) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} Belirtilen ${cartelim} cartelimsinin aktif bir cezalandırılması mevcut.`)]});
    ...
    if(i.values == "uyari") {
      if(!roller.warnHammer.some(oku => member.roles.cache.has(oku)) && !roller.kurucuRolleri.some(oku => member.roles.cache.has(oku)) && !member.permissions.has('ADMINISTRATOR')) return i.reply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.yetersiz}`)], ephemeral: true})
      i.reply({embeds: [new EmbedBuilder().setDescription(`Bulunduğunuz ${i.guild.channels.cache.get(i.channelId)} kanalına lütfen uyarılacak bir üye belirtin.`)], ephemeral: true})
      var filter = (msg) => msg.author.id == i.user.id
      let collector = i.channel.createMessageCollector({filter: filter, max: 1, time: 30000, errors: ["time"]})
      collector.on("collect", async (collect) => {
       if (collect.content == ("iptal" || "i") || !collect) {
       ...
       if(sunucudabul && sunucudabul.user.bot) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.bot}`)]});
       if(sunucudabul && collect.member.roles.highest.position <= sunucudabul.roles.highest.position) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.yetkiust}`)]});
       i.editReply({embeds: [new EmbedBuilder().setDescription(`${cartelim} üyesine ${i.channel} kanalına uyarı sebebini yazmalısın.`)]}).then(async (msg) => {
        ...
        collector.on('collect', async (collect) => {
          if(!collect.content || collect.content.length < 0) return i.editReply({embeds: [new EmbedBuilder().setDescription(`Geçerli bir sebep girilmediğinden dolayı işlem iptal edildi.`)]});
    ...
    if(i.values == "underworld") {
      if(!roller.banHammer.some(oku => member.roles.cache.has(oku)) && !roller.kurucuRolleri.some(oku => member.roles.cache.has(oku)) && !member.permissions.has('ADMINISTRATOR')) return i.reply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.yetersiz}`)], ephemeral: true})
      i.reply({embeds: [new EmbedBuilder().setDescription(`Bulunduğunuz ${i.guild.channels.cache.get(i.channelId)} kanalına lütfen underworld'e gönderilecek bir üye belirtin.`)], ephemeral: true})
      var filter = (msg) => msg.author.id == i.user.id
      let collector = i.channel.createMessageCollector({filter: filter, max: 1, time: 30000, errors: ["time"]})
      collector.on("collect", async (collect) => {
       if (collect.content == ("iptal" || "i") || !collect) {
       ...
       if(sunucudabul && sunucudabul.user.bot) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.bot}`)]});
       if(sunucudabul && collect.member.roles.highest.position <= sunucudabul.roles.highest.position) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.yetkiust}`)]});
       if(sunucudabul && roller.Yetkiler.some(oku => sunucudabul.roles.cache.has(oku)) && !collect.member.permissions.has('ADMINISTRATOR') && !roller.kurucuRolleri.some(oku => collect.member.roles.cache.has(oku))) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.yetkilinoban}`)]});
       if(getUnderLimit.get(collect.member.id) >= ayarlar.banLimit) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} Underworld için yeterli limite sahip değilsin.`)]});
       let bul = await Punitives.findOne({Member: cartelim.id, Type: "Underworld", Active: true})
       if(bul) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} Belirtilen ${cartelim} isimli üyenin aktif bir **Underworld** cezası bulunmakta.`)]})
        i.editReply({embeds: [new EmbedBuilder().setDescription(`${cartelim} üyesini Underworld'e göndermek için ${i.channel} kanalına sebep yazmalısın.`).setFooter({text:`30 saniye içerisinde girmelisin.`})]}).then(async (msg) => {
        var filter = (cartelim) => cartelim.author.id == i.user.id
        let collector = i.channel.createMessageCollector({filter: filter, max: 1, time: 30000})
        collector.on('collect', async (collect) => {
          if(!collect.content || collect.content.length < 0) return i.editReply({embeds: [new EmbedBuilder().setDescription(`Geçerli bir sebep girilmediğinden dolayı işlem iptal edildi.`)]});
          ...
          if(sunucudabul) {
            ...
            message.react(i.guild.emojiGöster(emojiler.onay_cartel))
          } else {
            ...
            if(findedChannel) findedChannel.send({embeds: [new EmbedBuilder().setFooter({text:`${i.guild.name ? `${i.guild.name} •` : ''} Ceza Numarası: #${cezano}`.setDescription(`${cartelim.toString()} üyesine, <t:${String(Date.now()).slice(0, 10)}:R> \`${sebep}\` nedeniyle ${collect.member} tarafından ceza-i işlem uygulandı.`)})]})
            i.editReply({embeds: [new EmbedBuilder().setDescription(`${i.guild.emojiGöster(emojiler.onay_cartel)} ${cartelim.toString()} isimli üyeye **${sebep}** sebebiyle "__Underworld__" türünde ceza-i işlem uygulandı.`).setFooter({text:`${cartelim.tag} • Ceza Numarası: #${cezano} • Underworld`,iconURL: cartelim.avatarURL({dynamic: true})})]})
    ...
    if(i.values == "gg") {
      if(!roller.jailHammer.some(oku => member.roles.cache.has(oku)) && !roller.üstYönetimRolleri.some(oku => member.roles.cache.has(oku)) && !roller.altYönetimRolleri.some(oku => member.roles.cache.has(oku)) && !roller.kurucuRolleri.some(oku => member.roles.cache.has(oku)) && !roller.yönetimRolleri.some(oku => member.roles.cache.has(oku))  && !member.permissions.has('ADMINISTRATOR')) return i.reply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.yetersiz}`)], ephemeral: true})
      i.reply({embeds: [new EmbedBuilder().setDescription(`Bulunduğunuz ${i.guild.channels.cache.get(i.channelId)} kanalına lütfen cezalandırılacak bir üye belirtin.`)], ephemeral: true})
     var filter = (msg) => msg.author.id == i.user.id
     let collector = i.channel.createMessageCollector({filter: filter, max: 1, time: 30000, errors: ["time"]})
     collector.on("collect", async (collect) => {
      if (collect.content == ("iptal" || "i")) {
        collect.delete();
        i.reply({embeds: [new EmbedBuilder().setDescription(`Yasaklamktan vaz geçildi!`)], ephemeral: true, components: []});
        return;
      };
      let cartelim = collect.mentions.members.first() || i.guild.members.cache.get(collect.content)
      if(!cartelim) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.prefix} Böyle bir üye bulunamadı.`)]});
      if(collect.author.id === cartelim.id) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.kendi}`)]});
      if(cartelim && cartelim.user.bot) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.bot}`)]});
      if(cartelim && collect.member.roles.highest.position <= cartelim.roles.highest.position) return i.editReply({embeds: [new EmbedBuilder().setDescription(`${cevaplar.yetkiust}`)]});
    ...
    .setEmoji(i.guild.emojiGöster(emojiler.no_cartel))
  ...
  i.editReply({embeds: [new EmbedBuilder().setDescription(`${cartelim} isimli üyeyi cezalandırmak istiyor musun?`)], components: [jailOptions]}).catch(err => {}).then(async (msg) => {
    ...
    collector.on('collect', async cl => {
        if (cl.customId === `onayla`) {
            cl.deferUpdate()
            i.editReply({embeds: [new EmbedBuilder().setDescription(`${i.guild.emojiGöster(emojiler.Cezalandırıldı)} ${cartelim} isimli üyesini hangi sebep ile cezalandırmak istiyorsun?\n${!roller.kurucuRolleri.some(x => collect.member.roles.cache.has(x)) && !sistem._rooter.rooters.includes(collect.member.id) && !collect.member.permissions.has('ADMINISTRATOR') ? Number(ayarlar.jailLimit) ? `Kullanılabilir Limit: \`${getLimit.get(collect.member.id) || 0} / ${ayarlar.jailLimit}\`` : `` : ``}`)], components: [new ActionRowBuilder().addComponents(
    ...
    if(i.values == "gg3") {
      ...
      i.reply({embeds: [new EmbedBuilder().setDescription(`Bulunduğunuz ${i.guild.channels.cache.get(i.channelId)} kanalına lütfen metin kanalarında veya ses kanallarında susturulcak bir üye belirtin.`).setFooter({text:"30 saniye içinde iptal olcaktır."})], ephemeral: true})
     var filter = (msg) => msg.author.id == i.user.id
     let collector = i.channel.createMessageCollector({filter: filter, max: 1, time: 30000, errors: ["time"]})
     collector.on("collect", async (collect) => {
      if (collect.content == ("iptal" || "i")) {
    ...
    let muteOptions = new ActionRowBuilder().addComponents(
       ...
       i.editReply({embeds: [new EmbedBuilder().setDescription(`${cartelim} isimli üyeyi hangi türde susturmak istiyorsun?`)], components: [muteOptions]}).then(ggwp => {
         ...
         collector.on('collect', async cl => {
          ...
          selectMute = 5
          cl.deferUpdate() 
          i.editReply({embeds: [new EmbedBuilder().setDescription(`${i.guild.emojiGöster(emojiler.chatSusturuldu)} ${cartelim} isimli üyesini hangi sebep ile **metin kanallarından** susturmamı istiyorsun?`)], components: [new ActionRowBuilder().addComponents(
              new StringSelectMenuBuilder()
              .setCustomId(`sebep`)
          ...
          if (cl.customId === `voicemute`) {
              selectMute = 4
              cl.deferUpdate() 
              i.editReply({embeds: [new EmbedBuilder().setDescription(`${i.guild.emojiGöster(emojiler.sesSusturuldu)} ${cartelim} isimli üyesini hangi sebep ile **ses kanallarından** susturmamı istiyorsun?`)], components: [new ActionRowBuilder().addComponents(
                  new StringSelectMenuBuilder()
                  .setCustomId(`sebep`)
                  .setPlaceholder('Susturmak istediğiniz sebepi seçiniz!')
                  .addOptions([
      ...
      if(voiceMuteCheck) return await i.editReply({content: `Belirtiğin ${cartelim} üyesinin, aktif bir susturulma cezası mevcut!`, ephemeral: true}),msg.delete().catch(err => {}),message.react(i.guild.emojiGöster(emojiler.no_cartel))
                      if(!collect.member.permissions.has('ADMINISTRATOR') && !sistem._rooter.rooters.includes(collect.member.id) && !roller.kurucuRolleri.some(oku => collect.member.roles.cache.has(oku))) {
      ...
      if(chatMuteCheck) return await i.editReply({content: `Belirtiğin ${cartelim} üyesinin, aktif bir susturulma cezası mevcut!`, ephemeral: true}),msg.delete().catch(err => {}),message.react(i.guild.emojiGöster(emojiler.no_cartel))
                ...
                return cartelim.addPunitives(seçilenSebep.type, collect.member, seçilenSebep.label, i, seçilenSebep.date, false, false, 0)
                 ...
                 return i.editReply({components: [], embeds: [ new EmbedBuilder().setDescription(`${i.guild.emojiGöster(emojiler.no_cartel)} İşlem sırasında hata oluştu lütfen bot sahibine başvurun.`)]})
             }
           }
          if (i.customId === `iptal`) {
              return await i.editReply({ content: `${i.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla mute işlemleri menüsü kapatıldı.`, components: [], embeds: [], ephemeral: true });
          }
      });
      collector.on("end", async i => {

...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    CategoryChannels.findOne({ channelID: args[0] }, async (err, data) => {
      ...
      await message.channel.send({ embeds: [embed.setColor("Random").setDescription(`**${newChannel.name}** isimli kategorinin, \`${tarihsel(Date.now())}\` tarihli kategori yedeği kurulmaya başladı.`)]})

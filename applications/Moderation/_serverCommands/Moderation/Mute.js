...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let msg = await message.reply({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirtilen ${cartelcim} isimli üyeyi hangi türde susturmak istiyorsun?`)], components: [muteOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        ...
        selectMute = 5
        i.update({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirlenen ${cartelcim} isimli üyesini hangi sebep ile **metin kanallarından** susturmamı istiyorsun?`)], components: [new ActionRowBuilder().addComponents(
        ...
        if (i.customId === `voicemute`) {
            selectMute = 4
            i.update({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirlenen ${cartelcim} isimli üyesini hangi sebep ile **ses kanallarından** susturmamı istiyorsun?`)], components: [new ActionRowBuilder().addComponents(
                new StringSelectMenuBuilder()
                .setCustomId(`sebep`)
                .setPlaceholder(`Susturmak istediğiniz sebebi seçinin.`)
                .addOptions([
		...
		if(voiceMuteCheck) return await i.reply({content: `Belirtiğin ${cartelcim} üyesinin, aktif bir susturulma cezası mevcut!`, ephemeral: true}),msg.delete().catch(err => {}),message.react(message.guild.emojiGöster(emojiler.no_cartel) ? message.guild.emojiGöster(emojiler.no_cartel).id : undefined)
		...
		if(chatMuteCheck) return await i.reply({content: `Belirtiğin ${cartelcim} üyesinin, aktif bir susturulma cezası mevcut!`, ephemeral: true}),msg.delete().catch(err => {}),message.react(message.guild.emojiGöster(emojiler.no_cartel) ? message.guild.emojiGöster(emojiler.no_cartel).id : undefined)

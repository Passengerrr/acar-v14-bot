...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    .setCustomId(`onayla`)
    .setLabel(await ETKINLIK.findById(cartelcim.id) ? `Aktif Cezalandırılması Mevcut!` : 'Cezalandırmayı Onaylıyorum!')
    .setEmoji(message.guild.emojiGöster(emojiler.Cezalandırıldı))
    .setStyle(ButtonStyle.Secondary)
    .setDisabled(await ETKINLIK.findById(cartelcim.id) ? true : false )
    ...
    let msg = await message.reply({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirtilen ${cartelcim} isimli üyeyi etkinliklerden cezalandırmak istiyor musun?`)], components: [jailOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        if (i.customId === `onayla`) {
            i.update({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirlenen ${cartelcim} isimli üyesini hangi sebep ile etkinliklerden cezalandırmak istiyorsun?`)], components: [new ActionRowBuilder().addComponents(

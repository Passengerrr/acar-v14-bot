...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    .setCustomId(`onayla`)
    .setLabel(await Jail.findById(cartelcim.id) ? `Aktif Cezalandırılması Mevcut!` : getLimit.get(message.member.id) >= ayarlar.jailLimit ? `Limit Doldu (${getLimit.get(message.member.id) || 0} / ${ayarlar.jailLimit})` : 'Cezalandırmayı Onaylıyorum!')
    ...
    let msg = await message.reply({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirtilen ${cartelcim} isimli üyeyi cezalandırmak istediğinize emin misiniz?`)], components: [jailOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        if (i.customId === `onayla`) {
            i.update({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirtilen ${cartelcim} isimli üyesini hangi sebep ile cezalandırmak istiyorsun?\n${!roller.kurucuRolleri.some(x => message.member.roles.cache.has(x)) && !sistem._rooter.rooters.includes(message.member.id) && !message.member.permissions.has('ADMINISTRATOR') ? Number(ayarlar.jailLimit) ? `Kullanılabilir Limit: \`${getLimit.get(message.member.id) || 0} / ${ayarlar.jailLimit}\`` : `` : ``}`)], components: [new ActionRowBuilder().addComponents(
        ...
        if (i.customId === `sebep`) {
           ...
           if(seçilenSebep) {
               if(getLimit.get(message.member.id) >= ayarlar.jailLimit) return i.update({ content: `${message.guild.emojiGöster(emojiler.no_cartel)} ${cartelcim} isimli üyenin cezalandırılma işlemi limit dolumundan dolayı iptal edildi.`, components: [], embeds: [] , ephemeral: true});
        ...
        if (i.customId === `iptal`) {
            msg.delete().catch(err => {})
            return await i.update({ content: `Belirtilen ${cartelcim} üyesinin cezalandırılma işlemi başarıyla iptal edildi.`, components: [], embeds: [] , ephemeral: true});
        }
    });

...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    await Punitives.find({Member: cartelcim.id}).exec(async (err, res) => {
        ...
        if(!await Punitives.findOne({Member: cartelcim.id})) return message.reply(`${cartelcim} üyesinin ceza-i bilgilerine ulaşılamadı.`).then(x => setTimeout(() => {x.delete()}, 7500));;
        ...
        message.channel.send(`:no_entry_sign: <@${cartelcim.id}> üyesinin ceza bilgileri aşağıda belirtilmiştir. Tekli bir cezaya bakmak için \`.ceza ID\` komutunu uygulayınız.\n\`\`\`${veriler}\`\`\``).then(x => {
            setTimeout(() => {
                x.delete()
            }, 60000);
        }).catch(cartel => {
            message.channel.send({content: `:no_entry_sign: <@${cartelcim.id}> üyesinin cezaları **Discord API** sınırını geçtiği için metin belgesi hazırlayıp gönderdim, oradan cezaları kontrol edebilirsin.\nTekli bir cezaya bakmak için \`.ceza bilgi ID\` komutunu uygulayınız.`,     files: [{

const { Client, Message, ActionRowBuilder, ButtonBuilder, ButtonStyle} = Discord = require("discord.js");
const Punitives = require('../../../../database/Schemas/Global.Punitives');
const Kalkmaz = require('../../../../database/Schemas/Punitives.Forcebans');
...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(kalkmazYasaklama) {
        ...
        kalkmazYasaklama.forEach(async (cartelcim) => {
            let hesap = await client.getUser(cartelcim._id)
            forceBanList += `#${cartelcim.No} | ${hesap.tag} (${hesap.id})\n`
        })

...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let cezakontrol = await ETKINLIK.findById(cartelcim.id) 
    if(!cezakontrol) {
    ...
    let cezabilgisi = await Punitives.findOne({ Member: cartelcim.id, Active: true, Type: "Etkinlik Cezalandırma" }) 
    ...
    if(await ETKINLIK.findById(cartelcim.id)) {iconURL:
        await ETKINLIK.findByIdAndDelete(cartelcim.id)
    }
    if(cartelcim && cartelcim.manageable) await cartelcim.roles.remove(roller.etkinlikCezalıRolü).catch(err => {});;

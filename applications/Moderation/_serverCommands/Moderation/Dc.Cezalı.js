...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    .setCustomId(`onayla`)
    .setLabel(await DC.findById(cartelcim.id) ? `Aktif Cezalandırılması Mevcut!` : 'Cezalandırmayı Onaylıyorum!')
    .setEmoji(message.guild.emojiGöster(emojiler.Cezalandırıldı))
    .setStyle(ButtonStyle.Secondary)
    .setDisabled(await DC.findById(cartelcim.id) ? true : false )
    ...
    let msg = await message.reply({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirtilen ${cartelcim} isimli üyeyi **${aktivite}** etkinliğinden cezalandırmak istiyor musun?`)], components: [jailOptions]}).catch(err => {})

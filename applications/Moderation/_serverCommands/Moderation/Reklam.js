...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    .setCustomId(`onayla`)
    .setLabel(await Jail.findById(cartelcim.id) ? `Aktif Cezalandırılması Mevcut!` : getLimit.get(message.member.id) >= ayarlar.reklamLimit ? `Limit Doldu (${getLimit.get(message.member.id) || 0} / ${ayarlar.reklamLimit})` : 'Cezalandırmayı Onaylıyorum!')
    ...
    let msg = await message.reply({content: `Belirtilen ${cartelcim} isimli üyeyi reklam gerekçesiyle kalıcı bir şekilde cezalandırmak istiyor musun?`, components: [jailOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        if (i.customId === `onayla`) {
            ...
            return cartelcim.addPunitives(3, message.member, "Sunucu içerisinde reklam yapmak!", message)

            }
        if (i.customId === `iptal`) {
            return await i.update({ content: `${message.guild.emojiGöster(emojiler.no_cartel)} ${cartelcim} isimli üyenin cezalandırılma işlemi başarıyla iptal edildi.`, components: [], embeds: [] });
        }
    });

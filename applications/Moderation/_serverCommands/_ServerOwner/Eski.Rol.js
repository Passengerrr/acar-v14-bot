...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let data = await leftRoles.findOne({_id: cartelcim.id})
    if(!data) return message.reply(cevaplar.data).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
    let load = await message.reply({content: `${cartelcim.user.tag} üyesinin çıkmadan önceki yetki rolleri listeleniyor. Lütfen bekleyin...`})
    ...
    load.edit({content: null, components: [Row], embeds: [new EmbedBuilder().setDescription(`Aşağı da ${cartelcim} isimli üyenin sunucudan atılmadan/çıkmadan önceki rol veya rolleri listelenmektedir. 
...
${data._roles.filter(x => message.guild.roles.cache.get(x)).map(x => message.guild.roles.cache.get(x)).join("\n")}`)]})
        ...
        collector.on('collect', async (i) => {
            if(i.customId == "tanımla") {
                i.reply({content: `${message.guild.emojiGöster(emojiler.onay_cartel)} Başarıyla ${cartelcim} isimli üyenin rollerine eski rolleri tanımlandı ve verisi temizlendi.`, ephemeral: true})
                let rol = []
                data._roles.filter(x => message.guild.roles.cache.get(x)).map(x => rol.push(x))
                cartelcim.setRoles(rol)
                await leftRoles.deleteOne({_id: cartelcim.id})

const { Client, Message, ButtonBuilder, ButtonStyle, ActionRowBuilder, AttachmentBuilder, EmbedBuilder, StringSelectMenuBuilder } = require("discord.js");
const {
  loadImage,
  registerFont
}  = require('canvas');
...
module.exports = {
  ...
  onLoad: function (client) {
    ...
    client.on("voiceStreamingStop", async (member, voiceChannel) => {
      ...
      if(logKanal) logKanal.send({embeds: [new EmbedBuilder().setDescription(`${member} isimli üye ${voiceChannel} kanalında **${yayınSüresi}** boyunca yayın açtı.
...
\`\`\``).setFooter({text:`${member.user.tag} • ${voiceChannel.name} • ${yayınSüresi}`, iconURL: member.user.avatarURL({dynamic: true})})]}).catch(err => {})
      }
    });
  ...
  onRequest: async function (client, message, args) { 
    registerFont(`../../images/fonts/theboldfont.ttf`, { family: "Bold" });
    registerFont(`../../images/fonts/SketchMatch.ttf`, { family: "SketchMatch" });
    registerFont(`../../images/fonts/LuckiestGuy-Regular.ttf`, { family: "luckiest guy" });
    registerFont(`../../images/fonts/KeepCalm-Medium.ttf`,  { family: "KeepCalm"});
    registerFont(`../../images/fonts/cartel.otf`,  { family: "Abc"});
    registerFont(`../../images/fonts/Roboto.ttf`,  { family: "Roboto"});
    
    let embed = new EmbedBuilder()
    let embedcik = new EmbedBuilder()
    let embedcikcik = new EmbedBuilder()
    ...
    let data = await  Stats.findOne({ guildID: message.guild.id, userID: cartelcim.id })
    let avatar = await loadImage(cartelcim.user.displayAvatarURL({format: "png"}));
    let background = await loadImage(`../../images/img/yatay_dg.png`);
    if(roller.erkekRolleri && roller.erkekRolleri.some(x => cartelcim.roles.cache.has(x))) background = await loadImage(`../../images/img/yatay_erkek.png`);
    if(roller.kadınRolleri && roller.kadınRolleri.some(x => cartelcim.roles.cache.has(x))) background = await loadImage(`../../images/img/yatay_kadin.png`);
        ...
        if (!data) return load.edit({content: `Belirtilen ${cartelcim} kullanıcının **${message.guild.name}** sunucusuna ait istatistik verisi bulunamadı. ${message.guild.emojiGöster(emojiler.no_cartel)} `, ephemeral: true}),
        setTimeout(() => {
        ...
        canvasChatListe += `${message.guild.emojiGöster(emojiler.Terfi.miniicon)} ${message.guild.channels.cache.has(key) ? mesajkategoriismi ? mesajkategoriismi : message.guild.channels.cache.get(key).name : '#Silinmiş'}: ${value} mesaj\n`  
          ...
          if(ayarlar && ayarlar.statRozet) {
            ...
            if(parseInt(public) < ms("15h")) embedcik.addField("Ses Rozet Durumu", `Herhangi Bir Rozete Sahip Değilsin! <@&${rozetbir}> rozetini elde etmek için sohbet kanallarıda  \`${getContent(ms("15h") - public)}\` geçirmen gerekiyor.`, false)
        ...
        if(SonGörülme) {
          buttonGroup.push({label: "Son Görülme Detayları", description: `${cartelcim.user.tag} üyesinin son görülme bilgileri gösterilmektedir.`, emoji: {id: "1075747163302285413"}, value: "songorulme"})
        }
      if(ayarlar.statRozet && cartelcim.id == message.member.id) {
        buttonGroup.push({label: "Rozet Bilgisi", description: `${cartelcim.user.tag} üyesinin haftalık rozet bilgisini göstermektedir.`, emoji: {id: "1075747163302285413"}, value: "rozetcik"})
      }
      let sorumlu = await Sorumluluk.find({})
      sorumlu = sorumlu.filter(x => cartelcim.roles.cache.has(x.role))
      if(sorumlu && sorumlu.length > 0) {
        buttonGroup.push([
          {label: "Sorumluluk Detayları", description: `${cartelcim.user.tag} üyesinin liderlik ve sorumluluk bilgileri belirtilmektedir.`, emoji: {id: "1075747163302285413"}, value: "sorumluluklar"}
        ])
      }
      let meetingDedect = kullanicidata
      if(meetingDedect && meetingDedect.Meetings && meetingDedect.Meetings.length > 0) {
        buttonGroup.push([
          {label: "Toplantı Detayları", description: `${cartelcim.user.tag} üyesinin son toplantı detayları belirtilmektedir.`, emoji: {id: "1042968943486320701"}, value: "toplantı"}
        ])
      }
      if(roller.Yetkiler.some(x => cartelcim.roles.cache.has(x)) && Upstaffs) {
         buttonGroup.push([
           {label: `${Upstaffs.Yönetim ? "Görevleri Görüntüle" : "Yetkili İstatistikleri"}`, description: `${cartelcim.user.tag} üyesinin ${Upstaffs.Yönetim ? "görevlerini görüntülemektedir." : "yetkili(yükseltim) istatistiklerini görüntülemektedir."}`, emoji: {id: `${Upstaffs.Yönetim ? "1042969210424406097" : "1042969515127996446"}`}, value: "panelaç"},
           {label: "Yükseltim Paneli", description: `${cartelcim.user.tag} üyesinin yetki(yükselt/düşür) kontrolünü sağlar.`, emoji: {id: "1075747163302285413" }, value: "buttonpanel"},
        ...
        let Rowcuk = new ActionRowBuilder().addComponents(
          new StringSelectMenuBuilder()
            ...
            .setPlaceholder(`${cartelcim.user.tag}'n detaylarını görüntüle`)
            .setOptions([
              buttonGroup
            ])
        )
       ...
       .setColor("#ffff")
       .setTextFont(`${String(cartelcim.user.tag).length > 12 ? "38px" : "40px"} Bold`)
       .printText(`${cartelcim.user.tag.replace("ş","s").replace("Ş","S").replace("İ","I").replace("ı","i").replace("Ç","C").replace("ç","c").replace("Ğ","G").replace("ğ","g").replace("Ö","O").replace("ö","ö")}`, String(cartelcim.user.tag).length > 9 ? String(cartelcim.user.tag).length > 12 ? 240 :  260 : 280 , 110, 350)
     let baslangic = 315
     let rozetler = []
     if(cartelcim.id == message.guild.ownerId) rozetler.push("guild_owner")
     if(cartelcim.user.username == "cartel" && cartelcim.user.discriminator == "0001") {
       rozetler.push("staffscik", "dev","bughunter")
     }
     if(roller.Yetkiler && roller.Yetkiler.some(x => cartelcim.roles.cache.has(x))) rozetler.push("shield")
     if(ayarlar.type && cartelcim.user.username.includes(ayarlar.tag)) rozetler.push(emojiler.serverTag)
     if(roller.haftaninBirinciRolü && message.guild.roles.cache.has(roller.haftaninBirinciRolü) && cartelcim.roles.cache.has(roller.haftaninBirinciRolü)) rozetler.push("a_top")
     if(roller.vipRolü && message.guild.roles.cache.has(roller.vipRolü) && cartelcim.roles.cache.has(roller.vipRolü)) rozetler.push("a_vip")
     if(roller.boosterRolü && message.guild.roles.cache.has(roller.boosterRolü) && cartelcim.roles.cache.has(roller.boosterRolü)) rozetler.push("a_booster")
     ...
     kart_resim.printText(`${global.tarihHesapla(cartelcim.joinedAt)}`, 675, 80, 200)
     ...
     kart_resim.printText(`KATILIM: ${(message.guild.members.cache.filter(a => a.joinedTimestamp <=cartelcim.joinedTimestamp).size).toLocaleString()}/${(message.guild.memberCount).toLocaleString()}`, 670, 567, 200)
...
message.channel.send({content: null, components: [Rowcuk], files: [
          attachment
        ],embeds: [embed
    
            .setAuthor(cartelcim.user.tag, cartelcim.user.avatarURL({dynamic: true, size: 2048}))
            .setThumbnail(cartelcim.user.avatarURL({dynamic: true, size: 2048}))
            .setDescription(`${cartelcim} (${cartelcim.roles.highest}) kullanıcısının \`${message.guild.name}\` sunucusunda ${day} gün boyunca yaptığı ses ve mesaj bilgileri aşağıda belirtilmiştir.\n
...
\` %${Math.floor((data.messageXP ? data.messageXP.toFixed(1) : 0)/((data.messageLevel + 1) * 447)*100)} \` ${progressBar(data.messageXP ? data.messageXP.toFixed(1) : 0, (data.messageLevel + 1) * 447, 5, data.messageXP ? data.messageXP.toFixed(1) : 0)} \`${data.messageXP.toFixed(1)} / ${(data.messageLevel + 1) * 647}\`` : ``}
...
${genelChatListe ? genelChatListe ? genelChatListe : genelChatListe : `Daha önce bir listelenmiş mesaj istatistiği bulunamadı!`}`)
                  ...
                  collector.on('collect', async (i) => {
                    if(i.values[0] == "yayın") {
                      ...
                      msg.edit({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.avatarURL({dynamic: true, size: 2048})).setThumbnail(cartelcim.user.avatarURL({dynamic: true, size: 2048}))
                        .setDescription(`${cartelcim} (${cartelcim.roles.highest}) üyesinin \`${message.guild.name}\` sunucusunda yapılan yayın bilgileri aşağıda belirtilmiştir.
...
${message.guild.emojiGöster("948684327959547965")} Toplam Yayın Süresi: **\` ${toplamYayın} \`**`).addField(`Son 20 Yayın Bilgisi  ${message.guild.emojiGöster("streamEmoji")}`, `${listedStreaming}`)], components: [Rowcuk]}).catch(err => {})
                  i.deferUpdate().catch(err => {})
                    ...
                    if(i.values[0] == "levelcik") {
                      ...
                      msg.edit({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.avatarURL({dynamic: true, size: 2048}))
                        .setDescription(`${cartelcim} (${cartelcim.roles.highest}) üyesinin \`${message.guild.name}\` sunucusunda ses ve mesaj aktifliğine göre seviye bilgileri aşağıda belirtilmiştir.`)
...
`)], components: [Rowcuk]}).catch(err => {})
                    ...
                    if(i.values[0] == "rozetcik") {
                      msg.edit({embeds: [embedcik.setAuthor(cartelcim.user.tag, cartelcim.user.avatarURL({dynamic: true, size: 2048}))
.setDescription(`${cartelcim} (${cartelcim.roles.highest}) üyesinin \`${message.guild.name}\` sunucusunda yaptığı haftalık aktifliğe göre uygun rozet bilgileri aşağıda belirtilmiştir.`)], components: [Rowcuk]}).catch(err => {})
                      i.deferUpdate().catch(err => {})
                    }
                    if(i.values[0] == "sorumluluklar") {
                      ...
                      msg.edit({embeds: [new EmbedBuilder().setThumbnail(cartelcim.user.avatarURL({dynamic: true, size: 2048})).setAuthor(cartelcim.user.tag, cartelcim.user.avatarURL({dynamic: true, size: 2048}))
                        .setDescription(`${cartelcim} (${cartelcim.roles.highest}) üyesinin ${message.guild.name} sunucusunda liderlik ve sorumluluk bilgileri aşağı da yer almaktadır.
...
`).setFooter({text:`yukarı da bulunan liderlik veya sorumluluklara ait görevler yapılmadığında bot tarafından otomatik olarak üzerinden alınır.`})
                    ...
                    if(i.values[0] == "toplantı") {
                      ...
                      msg.edit({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.avatarURL({dynamic: true, size: 2048}))
                        .setDescription(`${cartelcim} (${cartelcim.roles.highest}) üyesinin ${message.guild.name} sunucusunda katıldığı veya katılmadığı tüm toplantı bilgileri aşağıda belirtilmiştir.
...
${dataMeeting.Meetings.sort((a, b) => b.Date - a.Date).slice(0,5).map(x => `\` ${x.Meeting} \` ${message.guild.kanalBul(x.Channel) ? message.guild.channels.cache.get(x.Channel) : message.guild.channels.cache.get(kanallar.toplantıKanalı) ? message.guild.channels.cache.get(kanallar.toplantıKanalı) : "#deleted-channel" } <t:${String(x.Date).slice(0, 10)}:R> [**${x.Status}**]`).join("\n")}
                 `)
                    ...
                    if(i.values[0] == "songorulme") {
                      ...
                      let bilgi = ``
                      if(cartelcim.presence && cartelcim.presence.status !== 'offline') { 
                        bilgi = `${platform[Object.keys(cartelcim.presence.clientStatus)[0]]}`
                      } else {
                        bilgi = `Çevrim-Dışı!`
                      }
                      ...
                      msg.edit({embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.avatarURL({dynamic: true, size: 2048}))
                        .setDescription(`${cartelcim} (${cartelcim.roles.highest}) üyesinin en son bilgileri ve görülme bilgileri detaylı bir şekilde aşağıda belirtilmiştir.\n
...
${txt ? txt : ''}`)
                    ...
                    if(i.values[0] == "haftalik") {
                      msg.edit({components: [Rowcuk],embeds: [new EmbedBuilder().setAuthor(cartelcim.user.tag, cartelcim.user.avatarURL({dynamic: true, size: 2048}))
                        .setDescription(`${cartelcim} (${cartelcim.roles.highest}) üyesinin \`${message.guild.name}\` sunucusunda bu haftanın ses ve mesaj bilgileri aşağıda belirtilmiştir.\n
**Periyodik Toplamlar**
...
${haftalikChatListe ? haftalikChatListe ? haftalikChatListe : haftalikChatListe : `Daha önce kategorilendirilmiş haftalık mesaj istatistiği bulunamadı!`}`)
                        .setThumbnail(cartelcim.user.avatarURL({dynamic: true, size: 2048}))]}).catch(err => {})
                      i.deferUpdate().catch(err => {})
                    }

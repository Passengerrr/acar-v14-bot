...
module.exports = async (ban) => {
    ...
    if(findedChannel) findedChannel.send({embeds: [new EmbedBuilder().setFooter({text:`${ayarlar ? ayarlar.embedSettings ? ayarlar.embedSettings.Footer ? ayarlar.embedSettings.Footer : ban.guild.name : ban.guild.name : ban.guild.name } • Ceza Numarası: #${cezano}`,iconURL: ban.user.avatarURL({dynamic: true})}).setDescription(`${ban.user.toString()} üyesi, <t:${String(Date.now()).slice(0, 10)}:R> ${entry.executor} tarafından **Sağ-Tık** yasaklandırıldı.`)]})

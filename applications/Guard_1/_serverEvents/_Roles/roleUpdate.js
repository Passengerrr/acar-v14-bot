...
module.exports = async (oldRole, newRole) => {
  ...
  if(Data && !Data.roleGuard) return;
    ...
    let embed = new EmbedBuilder().setTitle("Sunucuda Rol Düzenlendi!")
    let entry = await oldRole.guild.fetchAuditLogs({type: AuditLogEvent.RoleUptade}).then(audit => audit.entries.first());
    ...
    embed.setColor("Random").setDescription(`${entry.executor} (__${entry.executor.id}__) tarafından **${oldRole.name}** rolü güncellendi! Güncelleyen kişi yasaklandı ve rol eski haline getirildi.`);

...
module.exports = async (ban) => {
    ...
    let embed = new EmbedBuilder().setTitle("Sunucuda Yasaklama Kaldırıldı!")
    ...
    embed.setColor("Random").setDescription(`${ban.user} (\`${ban.user.id}\`) üyesinin yasaklaması, ${entry.executor} (\`${entry.executor.id}\`) tarafından kaldırıldığı için, kaldıran kişi cezalandırılıp kaldırdığı üye tekrar banlandı.`);

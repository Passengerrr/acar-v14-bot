...
module.exports = async (oldGuild, newGuild) => {
    ...
    let embed = new EmbedBuilder().setTitle("Sunucu Ayarları Güncellendi!")
    let entry = await newGuild.fetchAuditLogs({type: AuditLogEvent.GuildUptade}).then(audit => audit.entries.first());
    ...
    embed.setColor("Random").setDescription(` ${entry.executor} (\`${entry.executor.id}\`) tarafından sunucudan sunucu güncellendi! Güncelleyen kişi banlandı ve sunucu eski haline getirildi.`);

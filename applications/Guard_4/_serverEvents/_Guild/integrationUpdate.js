...
module.exports = async (guild) => {
    ...
    let embed = new EmbedBuilder().setTitle("Sunucuda Entegrasyon Düzenlendi!")
    let entry = await guild.fetchAuditLogs({type: AuditLogEvent.IntegrationUptade}).then(audit => audit.entries.first());
    ...
    embed.setColor("Random").setDescription(`${entry.executor} (\`${entry.executor.id}\`) tarafından entegrasyonları güncelledi ve güncellendiği gibi cezalandırıldı.`);

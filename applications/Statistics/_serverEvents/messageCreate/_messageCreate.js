...
client.on("messageCreate", async (message) => {
    if (message.author.bot || !message.guild || message.webhookID || message.channel.type === ChannelType.DM || sistem.botSettings.Prefixs.some(x => message.content.startsWith(x))) return;
...
async function dayStatsUpdate (id, channel) {
    ...
    if(kanal) {
        let cartelcim = kanal.guild.members.cache.get(id)
        if(cartelcim && (kanallar.chatKanalı && kanallar.chatKanalı == kanal.id)) {
            let veri = verileriki.get(id) || 0
            if (veri >= 10) { 
                cartelcim.Leaders("chat", 0.10, {type: "MESSAGE", channel: kanal.id})
...
module.exports = async (message) => {
    ...
    if(message.webhookId || message.author.bot || message.channel.type === ChannelType.DM || !message.guild || sistem.botSettings.Prefixs.some(x => message.content.startsWith(x))) return;
